...
| 온도 | `org.pcsoft.framework.kunit.temperature` | 켈빈 (`KTemperatureUnit.BASE`) |
...
켈빈(기본), 섭씨, 화씨. 온도는 프레임워크의 **첫 번째(영구적인) 아핀 예외**입니다: 변환은 단일 계수가 아니라
오프셋과 스케일(`°C = K − 273.15`)입니다. 공유 엔진은 곱셈으로 유지되며, 아핀 변환은 `scaledBy`(`of` 뒤)와
`readBaseValue`(`into` 뒤) 훅을 통해 주입됩니다. 따라서 `25 of celsius`와 `t into fahrenheit`는 일반 동사를
사용하며 가려지기 쉬운 오버로드가 필요 없습니다. 값은 절대 켈빈으로 저장되고(`*`/`/`/`pow`가 그대로 동작),
이 그룹에는 **접두사가 없습니다**.
