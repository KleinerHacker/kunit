...
class KUnitFormatTest {
    ...
    fun `format speed into kilometers per hour`() {
        ...
        assertEquals("10.8 km/h", v.format(kilo.meters / hours, "%.1f", KLocale.EN_US))
    }
    ...
    fun `format with pattern and locale`() {
        val v = 3 of meters / seconds
        assertEquals("10,8 km/h", v.format(kilo.meters / hours, "%.1f", KLocale.DE_DE))
    }

    /** `toString(pattern)` renders the value's own base units with a formatted number. */
    @Test
    fun `toString with pattern on mixed unit`() {
        assertEquals("3.00 m/s", (3 of meters / seconds).toString("%.2f", KLocale.EN_US))
    }
    ...
    fun `toString with pattern on typed unit`() {
        assertEquals("1500.0 m", (1500 of meters).toString("%.1f", KLocale.EN_US))
    }
    ...
    fun `format invalid pattern fails`() {
        assertFailsWith<IllegalArgumentException> { (1 of meters).format(meters, "%.1d") }
    }
}
