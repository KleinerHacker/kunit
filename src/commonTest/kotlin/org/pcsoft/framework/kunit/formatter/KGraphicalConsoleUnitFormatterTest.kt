...
class KGraphicalConsoleUnitFormatterTest {
    ...
    private fun render(
        ...
        pattern: String = "%.1f",
    ) = KGraphicalConsoleUnitFormatter(config).format(KUnitFormatContext(value, units, pattern, KLocale.EN_US))
    ...
    fun `product colouring`() {
        ...
        assertEquals(
            ...
            KGraphicalConsoleUnitFormatter(config)
                .format(KUnitFormatContext(1.0, listOf(meter, perSecond3, perSecond2), "%.1f", KLocale.EN_US)),
        )
    }
    ...
    fun `fraction bar colouring`() {
        ...
        assertEquals(
            expected,
            KGraphicalConsoleUnitFormatter(config)
                .format(KUnitFormatContext(1.0, listOf(meter, perSecond), "%.1f", KLocale.EN_US)),
        )
    }
}
