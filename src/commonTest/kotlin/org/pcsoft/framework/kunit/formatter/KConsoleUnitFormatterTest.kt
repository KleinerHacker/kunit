...
class KConsoleUnitFormatterTest {
    ...
    fun `single unit`() {
        ...
        assertEquals(expected, (5 of meters).format(meters, "%.1f", KLocale.EN_US, KConsoleUnitFormatter()))
    }
    ...
    fun `fraction notation`() {
        ...
        assertEquals(expected, v.format(kilo.meters / hours, "%.1f", KLocale.EN_US, KConsoleUnitFormatter()))
    }
    ...
    fun `denominator exponent`() {
        ...
        assertEquals(expected, v.format(meters / (seconds pow 2), "%.2f", KLocale.EN_US, KConsoleUnitFormatter()))
    }
    ...
    fun `dimensionless value`() {
        val context = KUnitFormatContext(2.0, emptyList(), pattern = "%.1f", locale = KLocale.EN_US)
    ...
    fun `vivid palette`() {
        ...
        val context = KUnitFormatContext(5.0, listOf(KUnitTerm(KDistanceUnit.BASE, 1)), "%.1f", KLocale.EN_US)
    ...
    fun `monochrome palette leaves exponent uncoloured`() {
        ...
        val context = KUnitFormatContext(2.0, listOf(KUnitTerm(KDistanceUnit.BASE, 2)), "%.1f", KLocale.EN_US)
    ...
    fun `superscript exponent`() {
        ...
        val context = KUnitFormatContext(2.0, listOf(KUnitTerm(KTimeUnit.BASE, -1)), "%.1f", KLocale.EN_US)
        val expected = col(classic.numberColor, "2.0") + " " +
