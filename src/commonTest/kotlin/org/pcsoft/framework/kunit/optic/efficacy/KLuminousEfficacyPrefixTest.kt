...
class KLuminousEfficacyPrefixTest {
    ...
    @Test
    fun `si prefix on lumens per watt`() = forEachCase(prefixArgs()) { case ->
        `si prefix on lumens per watt`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on lumens per watt`(builder: KPrefixBuilder, factor: Double) {
