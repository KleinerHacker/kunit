...
/** All 24 SI prefix builders on the named radiance unit, each with its own assertion. */
class KRadiancePrefixTest {
    ...
    @Test
    fun `si prefix on watts per steradian square meter`() = forEachCase(prefixArgs()) { case ->
        `si prefix on watts per steradian square meter`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on watts per steradian square meter`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.wattsPerSteradianSquareMeter).value, rel(factor))
    }
}
