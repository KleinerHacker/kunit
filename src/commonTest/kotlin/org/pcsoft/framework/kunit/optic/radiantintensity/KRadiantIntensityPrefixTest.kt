...
/** All 24 SI prefix builders on the named radiant intensity unit, each with its own assertion. */
class KRadiantIntensityPrefixTest {
    ...
    @Test
    fun `si prefix on watts per steradian`() = forEachCase(prefixArgs()) { case ->
        `si prefix on watts per steradian`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on watts per steradian`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.wattsPerSteradian).value, rel(factor))
    }
}
