...
class KAngularVelocityUnitTest {
    ...
    fun `named tokens in radians per second`() {
        assertEquals(2.0 * PI / 60.0, (1 of revolutionsPerMinute).value, 1e-12)
        assertEquals(2.0 * PI, (1 of revolutionsPerSecond).value, 1e-12)
    ...
    fun `enum entries expose symbol and factor`() {
        ...
        assertEquals(PI / 180.0, KAngularVelocityUnit.DEGREES_PER_SECOND.baseValue, 1e-12)
        assertEquals(2.0 * PI / 60.0, KAngularVelocityUnit.REVOLUTIONS_PER_MINUTE.baseValue, 1e-12)
        assertEquals(2.0 * PI, KAngularVelocityUnit.REVOLUTIONS_PER_SECOND.baseValue, 1e-12)
    }
