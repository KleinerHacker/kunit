...
class KMassPrefixTest {
    ...
    @Test
    fun `si prefix on grams`() = forEachCase(prefixArgs()) { case ->
        `si prefix on grams`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on grams`(builder: KPrefixBuilder, factor: Double) {
