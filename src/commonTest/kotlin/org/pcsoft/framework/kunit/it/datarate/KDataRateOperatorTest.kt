...
class KDataRateOperatorTest {
    ...
    @Test
    fun `storage over time is data rate`() = forEachCase(matrix()) { case ->
        `storage over time is data rate`(case[0] as KStorageUnitInstance, case[1] as Double, case[2] as KTimeUnitInstance, case[3] as Double)
    }

    private fun `storage over time is data rate`(s: KStorageUnitInstance, sb: Double, t: KTimeUnitInstance, tb: Double) {
    ...
    @Test
    fun `rate times time is storage`() = forEachCase(matrix()) { case ->
        `rate times time is storage`(case[0] as KStorageUnitInstance, case[1] as Double, case[2] as KTimeUnitInstance, case[3] as Double)
    }

    private fun `rate times time is storage`(s: KStorageUnitInstance, sb: Double, t: KTimeUnitInstance, tb: Double) {
        val r = (100 of s) / (5 of t)
        val amount = r * (5 of t)
