...
class KStoragePrefixTest {
    ...
    @Test
    fun `si prefix`() = forEachCase(siArgs()) { case ->
        `si prefix`(case[0] as KAugmentingPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix`(builder: KAugmentingPrefixBuilder, factor: Double) {
    ...
    @Test
    fun `binary prefix`() = forEachCase(binaryArgs()) { case ->
        `binary prefix`(case[0] as KStorageBinaryPrefixBuilder, case[1] as Double)
    }

    private fun `binary prefix`(builder: KStorageBinaryPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.bytes).value, rel(factor))
    }
