...
class KEnergyUnitPrefixTest {
    ...
    @Test
    fun `si prefix on joules`() = forEachCase(prefixArgs()) { case ->
        `si prefix on joules`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on joules`(builder: KPrefixBuilder, factor: Double) {
