...
class KEnergyUnitTest {
    ...
    fun `unit conversion`() = forEachCase(tokenArgs()) { case ->
        `unit conversion`(case[0] as String, case[1] as KEnergyUnitInstance, case[2] as KEnergyUnit)
    }

    private fun `unit conversion`(name: String, token: KEnergyUnitInstance, unit: KEnergyUnit) {
