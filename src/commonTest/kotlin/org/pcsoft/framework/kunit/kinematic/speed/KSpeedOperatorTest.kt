...
class KSpeedOperatorTest {
    ...
    @Test
    fun `length over time is speed`() = forEachCase(matrix()) { case ->
        `length over time is speed`(case[0] as KLengthUnitInstance, case[1] as Double, case[2] as KTimeUnitInstance, case[3] as Double)
    }

    private fun `length over time is speed`(l: KLengthUnitInstance, lb: Double, t: KTimeUnitInstance, tb: Double) {
    ...
    @Test
    fun `speed times time is length`() = forEachCase(matrix()) { case ->
        `speed times time is length`(case[0] as KLengthUnitInstance, case[1] as Double, case[2] as KTimeUnitInstance, case[3] as Double)
    }

    private fun `speed times time is length`(l: KLengthUnitInstance, lb: Double, t: KTimeUnitInstance, tb: Double) {
        val v = (10 of l) / (2 of t)
        val length = v * (2 of t)
