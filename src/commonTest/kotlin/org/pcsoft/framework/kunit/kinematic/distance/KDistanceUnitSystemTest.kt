...
class KDistanceUnitSystemTest {
    ...
    @Test
    fun `construction and round-trip read`() = forEachCase(tokens()) { case ->
        `construction and round-trip read`(case[0] as KLengthUnitInstance, case[1] as Double)
    }

    private fun `construction and round-trip read`(token: KLengthUnitInstance, base: Double) {
        val v = 7 of token
