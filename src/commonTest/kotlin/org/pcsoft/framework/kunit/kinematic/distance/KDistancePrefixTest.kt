...
class KDistancePrefixTest {
    ...
    @Test
    fun `prefix standalone on meters`() = forEachCase(prefixes()) { case ->
        `prefix standalone on meters`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `prefix standalone on meters`(builder: KPrefixBuilder, factor: Double) {
    ...
    @Test
    fun `prefix times unit`() = forEachCase(tokens()) { case ->
        `prefix times unit`(case[0] as KLengthUnitInstance, case[1] as Double)
    }

    private fun `prefix times unit`(@Suppress("UNUSED_PARAMETER") token: KLengthUnitInstance, base: Double) {
