...
class KLengthLightUnitTest {
    ...
    @Test
    fun `light token builds and reads`() = forEachCase(tokens()) { case ->
        `light token builds and reads`(case[0] as KLengthUnitInstance, case[1] as Double)
    }

    private fun `light token builds and reads`(token: KLengthUnitInstance, base: Double) {
