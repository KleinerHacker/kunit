...
class KFrequencyPrefixTest {
    ...
    @Test
    fun `si prefix on hertz`() = forEachCase(prefixArgs()) { case ->
        `si prefix on hertz`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on hertz`(builder: KPrefixBuilder, factor: Double) {
