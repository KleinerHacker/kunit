...
class KResistanceUnitTest {
    ...
    fun `unit conversion`() = forEachCase(tokenArgs()) { case ->
        `unit conversion`(case[0] as String, case[1] as KResistanceUnitInstance, case[2] as KResistanceUnit)
    }

    private fun `unit conversion`(name: String, token: KResistanceUnitInstance, unit: KResistanceUnit) {
