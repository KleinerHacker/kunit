...
class KReluctanceUnitSystemTest {
    ...
    fun `construction and round-trip`() = forEachCase(tokenArgs()) { case ->
        `construction and round-trip`(case[0] as KReluctanceUnitInstance, case[1] as Double)
    }

    private fun `construction and round-trip`(token: KReluctanceUnitInstance, base: Double) {
