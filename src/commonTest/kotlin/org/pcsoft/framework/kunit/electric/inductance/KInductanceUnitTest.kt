...
class KInductanceUnitTest {
    ...
    fun `unit conversion`() = forEachCase(tokenArgs()) { case ->
        `unit conversion`(case[0] as String, case[1] as KInductanceUnitInstance, case[2] as KInductanceUnit)
    }

    private fun `unit conversion`(name: String, token: KInductanceUnitInstance, unit: KInductanceUnit) {
