...
class KPermeabilityUnitPrefixTest {
    ...
    @Test
    fun `si prefix on henries per meter`() = forEachCase(prefixArgs()) { case ->
        `si prefix on henries per meter`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on henries per meter`(builder: KPrefixBuilder, factor: Double) {
