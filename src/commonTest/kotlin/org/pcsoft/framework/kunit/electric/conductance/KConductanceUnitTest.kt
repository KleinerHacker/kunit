...
class KConductanceUnitTest {
    ...
    fun `unit conversion`() = forEachCase(tokenArgs()) { case ->
        `unit conversion`(case[0] as String, case[1] as KConductanceUnitInstance, case[2] as KConductanceUnit)
    }

    private fun `unit conversion`(name: String, token: KConductanceUnitInstance, unit: KConductanceUnit) {
