...
class KReluctanceUnitTest {
    ...
    fun `unit conversion`() = forEachCase(tokenArgs()) { case ->
        `unit conversion`(case[0] as String, case[1] as KReluctanceUnitInstance, case[2] as KReluctanceUnit)
    }

    private fun `unit conversion`(name: String, token: KReluctanceUnitInstance, unit: KReluctanceUnit) {
