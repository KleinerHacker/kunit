...
class KElectricMobilityUnitSystemTest {
    ...
    fun `construction and round-trip`() = forEachCase(tokenArgs()) { case ->
        `construction and round-trip`(case[0] as KElectricMobilityUnitInstance, case[1] as Double)
    }

    private fun `construction and round-trip`(token: KElectricMobilityUnitInstance, base: Double) {
