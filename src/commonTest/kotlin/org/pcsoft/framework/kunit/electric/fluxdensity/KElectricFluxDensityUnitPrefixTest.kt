...
class KElectricFluxDensityUnitPrefixTest {
    ...
    @Test
    fun `si prefix on coulombs per square meter`() = forEachCase(prefixArgs()) { case ->
        `si prefix on coulombs per square meter`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on coulombs per square meter`(builder: KPrefixBuilder, factor: Double) {
