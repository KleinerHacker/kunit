...
class KElectricCurrentPrefixTest {
    ...
    @Test
    fun `si prefix on amperes`() = forEachCase(prefixArgs()) { case ->
        `si prefix on amperes`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on amperes`(builder: KPrefixBuilder, factor: Double) {
