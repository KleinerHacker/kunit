...
/**
 * The SI prefix builders on the conductance units. Conductance accepts *any* magnitude, so all 24 SI
 * prefixes are available on every unit (`milli.siemens` = mS, `kilo.siemens` = kS). Covers the prefixed
 * extension property.
 */
class KConductanceUnitPrefixTest {
    ...
    /** Each SI prefix scales a siemens template by its factor (`kilo.siemens == 1000 S`). */
    @Test
    fun `si prefix on siemens`() = forEachCase(prefixArgs()) { case ->
        `si prefix on siemens`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on siemens`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.siemens).value, rel(factor))
    }

    /** Every named conductance unit exposes its own prefixed extension property (`milli.mhos`, …). */
    @Test
