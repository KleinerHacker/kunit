...
class KMagneticFluxDensityUnitPrefixTest {
    ...
    @Test
    fun `si prefix on teslas`() = forEachCase(prefixArgs()) { case ->
        `si prefix on teslas`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on teslas`(builder: KPrefixBuilder, factor: Double) {
