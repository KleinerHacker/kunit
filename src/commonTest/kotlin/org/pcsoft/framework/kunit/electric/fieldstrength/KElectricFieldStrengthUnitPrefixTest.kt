...
class KElectricFieldStrengthUnitPrefixTest {
    ...
    @Test
    fun `si prefix on volts per meter`() = forEachCase(prefixArgs()) { case ->
        `si prefix on volts per meter`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on volts per meter`(builder: KPrefixBuilder, factor: Double) {
