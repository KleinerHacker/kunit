...
class KInductanceUnitPrefixTest {
    ...
    @Test
    fun `si prefix on henries`() = forEachCase(prefixArgs()) { case ->
        `si prefix on henries`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on henries`(builder: KPrefixBuilder, factor: Double) {
