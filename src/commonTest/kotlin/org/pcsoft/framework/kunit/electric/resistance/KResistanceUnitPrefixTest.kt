...
class KResistanceUnitPrefixTest {
    ...
    @Test
    fun `si prefix on ohms`() = forEachCase(prefixArgs()) { case ->
        `si prefix on ohms`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on ohms`(builder: KPrefixBuilder, factor: Double) {
