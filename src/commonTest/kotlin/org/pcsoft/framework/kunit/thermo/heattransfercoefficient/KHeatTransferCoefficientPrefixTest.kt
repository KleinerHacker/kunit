...
class KHeatTransferCoefficientPrefixTest {
    ...
    @Test
    fun `si prefix on watts per square meter kelvin`() = forEachCase(prefixArgs()) { case ->
        `si prefix on watts per square meter kelvin`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on watts per square meter kelvin`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.wattsPerSquareMeterKelvin).value, rel(factor))
    }

    @Test
    fun `si prefix on btus per hour square foot fahrenheit`() = forEachCase(prefixArgs()) { case ->
        `si prefix on btus per hour square foot fahrenheit`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on btus per hour square foot fahrenheit`(builder: KPrefixBuilder, factor: Double) {
    ...
    @Test
    fun `si prefix on calories per second square centimeter kelvin`() = forEachCase(prefixArgs()) { case ->
        `si prefix on calories per second square centimeter kelvin`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on calories per second square centimeter kelvin`(builder: KPrefixBuilder, factor: Double) {
