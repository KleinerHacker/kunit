...
/** All 24 SI prefix builders on every named thermal resistance unit, each with its own assertion. */
class KThermalResistancePrefixTest {
    ...
    @Test
    fun `si prefix on kelvins per watt`() = forEachCase(prefixArgs()) { case ->
        `si prefix on kelvins per watt`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on kelvins per watt`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.kelvinsPerWatt).value, rel(factor))
    }

    @Test
    fun `si prefix on degrees celsius per watt`() = forEachCase(prefixArgs()) { case ->
        `si prefix on degrees celsius per watt`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on degrees celsius per watt`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.degreesCelsiusPerWatt).value, rel(factor))
    }

    @Test
    fun `si prefix on hour fahrenheit per btu`() = forEachCase(prefixArgs()) { case ->
        `si prefix on hour fahrenheit per btu`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on hour fahrenheit per btu`(builder: KPrefixBuilder, factor: Double) {
