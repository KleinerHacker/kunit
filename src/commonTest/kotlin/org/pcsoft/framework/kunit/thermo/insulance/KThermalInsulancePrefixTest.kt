...
/** All 24 SI prefix builders on every named thermal insulance unit, each with its own assertion. */
class KThermalInsulancePrefixTest {
    ...
    @Test
    fun `si prefix on square meter kelvin per watt`() = forEachCase(prefixArgs()) { case ->
        `si prefix on square meter kelvin per watt`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on square meter kelvin per watt`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.squareMeterKelvinPerWatt).value, rel(factor))
    }

    @Test
    fun `si prefix on imperial r value`() = forEachCase(prefixArgs()) { case ->
        `si prefix on imperial r value`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on imperial r value`(builder: KPrefixBuilder, factor: Double) {
    ...
    @Test
    fun `si prefix on clo`() = forEachCase(prefixArgs()) { case ->
        `si prefix on clo`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on clo`(builder: KPrefixBuilder, factor: Double) {
        val expected = factor * 0.155
        assertEquals(expected, (1 of builder.clo).value, rel(expected))
    }

    @Test
    fun `si prefix on tog`() = forEachCase(prefixArgs()) { case ->
        `si prefix on tog`(case[0] as KPrefixBuilder, case[1] as Double)
    }

    private fun `si prefix on tog`(builder: KPrefixBuilder, factor: Double) {
