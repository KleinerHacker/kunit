 ...
 * m²/s ([KThermalDiffusivityUnit.BASE]).
 *
 ...
 * `α = λ / (ρ · c_p)`; see `KThermalDiffusivityUnitOperators.kt`.
 *
 ...
 * ```
 */
class KThermalDiffusivityUnitInstance internal constructor(internal val instance: KMixedUnitInstance) :
    KUnitInstance<KThermalDiffusivityUnitInstance>, KUnitMeasurable by instance {

    /** Returns a new thermal diffusivity with [value] (m²/s) scaled by [factor]. */
    override fun scaledBy(factor: Double): KThermalDiffusivityUnitInstance =
        thermalDiffusivityInstanceOf(value * factor)

    /**
     * Adds two thermal diffusivities, automatically converting between different
     * [KThermalDiffusivityUnit]s since both operands are always normalized to m²/s internally.
     */
    override operator fun plus(other: KThermalDiffusivityUnitInstance): KThermalDiffusivityUnitInstance =
        thermalDiffusivityInstanceOf(value + other.value)

    /** Subtracts two thermal diffusivities. See [plus] for the automatic unit conversion. */
    override operator fun minus(other: KThermalDiffusivityUnitInstance): KThermalDiffusivityUnitInstance =
        thermalDiffusivityInstanceOf(value - other.value)

    /** Multiplies two thermal diffusivities, producing a new [KMixedUnitInstance]. */
    operator fun times(other: KThermalDiffusivityUnitInstance): KMixedUnitInstance = instance * other.instance

    /** Divides two thermal diffusivities, producing a new (dimensionless) [KMixedUnitInstance]. */
    operator fun div(other: KThermalDiffusivityUnitInstance): KMixedUnitInstance = instance / other.instance

    /** Compares two thermal diffusivities by their normalized [value] (m²/s). */
    override operator fun compareTo(other: KThermalDiffusivityUnitInstance): Int = value.compareTo(other.value)

    /** Structural equality by normalized [value] (m²/s). */
    override fun equals(other: Any?): Boolean = other is KThermalDiffusivityUnitInstance && value == other.value

    override fun hashCode(): Int = value.hashCode()

    /** Base-unit representation, e.g. `"1.11E-4 m²/s"`. */
    override fun toString(): String = "$value ${KThermalDiffusivityUnit.BASE.symbol}"
}

// --- Factory helper (single creation source; constructor stays internal) -------------------------

/**
 * Builds a [KThermalDiffusivityUnitInstance] from a value already expressed in m²/s
 * ([KThermalDiffusivityUnit.BASE]).
 *
 ...
 */
internal fun thermalDiffusivityInstanceOf(squareMetersPerSecond: Double): KThermalDiffusivityUnitInstance =
...
/** Builds a value-1 [KThermalDiffusivityUnitInstance] for the given [unit]. */
internal fun thermalDiffusivityOfUnit(unit: KThermalDiffusivityUnit): KThermalDiffusivityUnitInstance =
    thermalDiffusivityInstanceOf(unit.baseValue)
 ...
 * diffusivity.
 */
fun KMixedUnitInstance.toThermalDiffusivity(): KThermalDiffusivityUnitInstance {
    ...
    return thermalDiffusivityInstanceOf(normalized)
}
