 ...
 * @property pattern an optional number pattern applied to [value] only (e.g. `"%.1f"`, see [formatNumber]
 * for the supported grammar); `null` renders the plain [Double.toString].
 * @property locale the locale used when [pattern] is applied (decimal separator etc.); defaults to
 * [KLocale.ROOT], so unqualified formatting is identical on every platform.
 */
 ...
 * [KUnitFormatContext.pattern] is `null`, otherwise the pattern applied under the context's
 * [KUnitFormatContext.locale]. The unformatted form is rendered portably (`5.0`, not JS's `5`), so the
 ...
 *
 * @throws IllegalArgumentException if [KUnitFormatContext.pattern] is not a valid number pattern.
 */
fun KUnitFormatContext.renderValue(): String {
    if (pattern == null) return renderDouble(value)
