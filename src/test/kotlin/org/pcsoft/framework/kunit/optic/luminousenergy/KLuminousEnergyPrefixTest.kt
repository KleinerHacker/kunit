...
class KLuminousEnergyPrefixTest {
    ...
    fun `si prefix on lumen seconds`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.lumenSeconds).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on talbots`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.talbots).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on lumen hours`(builder: KPrefixBuilder, factor: Double) {
