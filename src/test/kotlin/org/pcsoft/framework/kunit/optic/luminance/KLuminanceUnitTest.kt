...
class KLuminanceUnitTest {
    ...
    fun `named tokens in candelas per square meter`() {
        ...
        assertEquals(1.0 / Math.PI, (1 of apostilbs) into candelasPerSquareMeter, 1e-12)
        assertEquals(1.0e4 / Math.PI, (1 of lamberts) into candelasPerSquareMeter, 1e-9)
    ...
    fun `enum entries expose symbol and base value`() {
        ...
        assertEquals(1.0 / Math.PI, KLuminanceUnit.APOSTILB.baseValue, 1e-12)
        assertEquals("L", KLuminanceUnit.LAMBERT.symbol)
        assertEquals(1.0e4 / Math.PI, KLuminanceUnit.LAMBERT.baseValue, 1e-9)
