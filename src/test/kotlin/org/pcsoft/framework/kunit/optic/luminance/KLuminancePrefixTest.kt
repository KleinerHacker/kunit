...
class KLuminancePrefixTest {
    ...
    fun `si prefix on candelas per square meter`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.candelasPerSquareMeter).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on nits`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.nits).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on stilbs`(builder: KPrefixBuilder, factor: Double) {
    ...
    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on apostilbs`(builder: KPrefixBuilder, factor: Double) {
        val expected = factor / Math.PI
        assertEquals(expected, (1 of builder.apostilbs).value, rel(expected))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on lamberts`(builder: KPrefixBuilder, factor: Double) {
        val expected = factor * 1.0e4 / Math.PI
        assertEquals(expected, (1 of builder.lamberts).value, rel(expected))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on foot lamberts`(builder: KPrefixBuilder, factor: Double) {
        val expected = factor * 10.763910416709722 / Math.PI
