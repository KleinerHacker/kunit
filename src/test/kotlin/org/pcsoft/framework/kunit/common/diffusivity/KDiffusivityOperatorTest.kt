...
class KDiffusivityOperatorTest {
    ...
    fun `diffusivity from conductivity density and specific heat`() {
        ...
        assertIs<KDiffusivityUnitInstance>(alpha)
     ...
     * expression through `toDiffusivity()` - yield the same typed, value-equal result.
     */
    @Test
    fun `all decompositions agree`() {
        ...
        assertIs<KDiffusivityUnitInstance>(typed)
        assertIs<KDiffusivityUnitInstance>(native)
