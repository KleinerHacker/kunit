...
class KTorqueOperatorTest {
    ...
    fun `torque times angular velocity is power`() {
        ...
        assertEquals(200.0 * 2.0 * Math.PI, p1 into watts, 1e-6)
        assertEquals(200.0 * 2.0 * Math.PI, p2 into watts, 1e-6)
    }
    ...
    fun `power over torque is angular velocity`() {
        val omega: KAngularVelocityUnitInstance = (400.0 * Math.PI of watts) / (200 of joules)
