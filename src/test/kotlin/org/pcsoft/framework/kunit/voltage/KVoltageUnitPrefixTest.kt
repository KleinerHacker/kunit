...
class KVoltageUnitPrefixTest {
    ...
    fun `si prefix on volts`(builder: KPrefixBuilder, factor: Double) {
    ...
    fun `every voltage unit has a prefixed property`() {
        ...
        assertEquals(f * KVoltageUnit.STATVOLT.baseValue, (1 of kilo.statvolts).value, rel(f * KVoltageUnit.STATVOLT.baseValue))
        assertEquals(f * KVoltageUnit.ABVOLT.baseValue, (1 of kilo.abvolts).value, rel(f * KVoltageUnit.ABVOLT.baseValue))
        assertEquals(f * KVoltageUnit.WESTON_CELL.baseValue, (1 of kilo.westonCells).value, rel(f * KVoltageUnit.WESTON_CELL.baseValue))
        assertEquals(f * KVoltageUnit.DANIELL.baseValue, (1 of kilo.daniells).value, rel(f * KVoltageUnit.DANIELL.baseValue))
    }
}
