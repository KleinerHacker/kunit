...
class KUnitInstanceTest {
    ...
    fun `times introduces new unit with its own exponent`() {
        ...
        val timeSquared = KUnitInstance(2.0, listOf(KUnitTerm(KTimeUnit.SECOND, -2)))

        val result = distance * timeSquared

        assertEquals(20.0, result.value)
        assertTrue(result.hasSameUnits(KUnitInstance(0.0, listOf(KUnitTerm(KLengthUnit.METER, 1), KUnitTerm(KTimeUnit.SECOND, -2)))))
    }

    @Test
    fun `times cancels out matching unit to zero exponent`() {
        val speed = KUnitInstance(10.0, listOf(KUnitTerm(KLengthUnit.METER, 1), KUnitTerm(KTimeUnit.SECOND, 1)))
        val inverseTime = KUnitInstance(1.0, listOf(KUnitTerm(KTimeUnit.SECOND, -1)))
    ...
    fun `div subtracts exponent of matching unit`() {
        val distance = KUnitInstance(10.0, listOf(KUnitTerm(KLengthUnit.METER, 1)))
        val time = KUnitInstance(2.0, listOf(KUnitTerm(KTimeUnit.SECOND, 1)))
    ...
    fun `plus fails when unit groups differ`() {
        val a = KUnitInstance(5.0, listOf(KUnitTerm(KLengthUnit.METER, 1)))
        val b = KUnitInstance(3.0, listOf(KUnitTerm(KTimeUnit.SECOND, 1)))
    ...
    fun `equals and hashCode are structural and order independent`() {
        val a = KUnitInstance(5.0, listOf(KUnitTerm(KLengthUnit.METER, 1), KUnitTerm(KTimeUnit.SECOND, -1)))
        val b = KUnitInstance(5.0, listOf(KUnitTerm(KTimeUnit.SECOND, -1), KUnitTerm(KLengthUnit.METER, 1)))
        val c = KUnitInstance(9.0, listOf(KUnitTerm(KLengthUnit.METER, 1), KUnitTerm(KTimeUnit.SECOND, -1)))
    ...
    fun `hasSameUnits is order independent`() {
        val a = KUnitInstance(5.0, listOf(KUnitTerm(KLengthUnit.METER, 1), KUnitTerm(KTimeUnit.SECOND, -1)))
        val b = KUnitInstance(9.0, listOf(KUnitTerm(KTimeUnit.SECOND, -1), KUnitTerm(KLengthUnit.METER, 1)))
    ...
    fun `valueAs converts speed to km per hour using mixed targets in any order`() {
        // 10 m / 1 s
        val speedInstance = KUnitInstance(10.0, listOf(KUnitTerm(KLengthUnit.METER, 1), KUnitTerm(KTimeUnit.SECOND, -1)))
    ...
    fun `toString without targets uses raw units`() {
        val instance = KUnitInstance(5.0, listOf(KUnitTerm(KLengthUnit.METER, 1), KUnitTerm(KTimeUnit.SECOND, -1)))
