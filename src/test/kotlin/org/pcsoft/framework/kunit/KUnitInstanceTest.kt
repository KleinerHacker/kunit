...
import org.pcsoft.framework.kunit.length.LengthDerivedUnit
import org.pcsoft.framework.kunit.length.LengthUnit
...
class KUnitInstanceTest {
    ...
    fun `times merges exponent of matching unit`() {
        ...
        assertEquals(listOf(KUnitTerm(LengthUnit.METER, 2)), result.units)
    }

    @Test
    fun `times introduces new unit with its own exponent`() {
        val distance = KUnitInstance(10.0, listOf(KUnitTerm(LengthUnit.METER, 1)))
        ...
        assertTrue(result.hasSameUnits(KUnitInstance(0.0, listOf(KUnitTerm(LengthUnit.METER, 1), KUnitTerm(TimeUnit.SECOND, -2)))))
    }

    @Test
    fun `times cancels out matching unit to zero exponent`() {
        val speed = KUnitInstance(10.0, listOf(KUnitTerm(LengthUnit.METER, 1), KUnitTerm(TimeUnit.SECOND, 1)))
        val inverseTime = KUnitInstance(1.0, listOf(KUnitTerm(TimeUnit.SECOND, -1)))

        val result = speed * inverseTime

        assertEquals(listOf(KUnitTerm(LengthUnit.METER, 1)), result.units)
    }
    ...
    fun `div subtracts exponent of matching unit`() {
        ...
        assertEquals(setOf(KUnitTerm(LengthUnit.METER, 1), KUnitTerm(TimeUnit.SECOND, -1)), speed.units.toSet())
    }
    ...
    fun `plus fails when units differ`() {
        val a = KUnitInstance(5.0, listOf(KUnitTerm(LengthUnit.METER, 1)))
        val b = KUnitInstance(3.0, listOf(KUnitTerm(LengthUnit.MILE, 1)))
    ...
    fun `plus fails when exponents differ`() {
        val area = KUnitInstance(5.0, listOf(KUnitTerm(LengthUnit.METER, 2)))
        val length = KUnitInstance(3.0, listOf(KUnitTerm(LengthUnit.METER, 1)))
    ...
    fun `equals and hashCode are structural and order independent`() {
        ...
        val b = KUnitInstance(5.0, listOf(KUnitTerm(TimeUnit.SECOND, -1), KUnitTerm(LengthUnit.METER, 1)))
        val c = KUnitInstance(9.0, listOf(KUnitTerm(LengthUnit.METER, 1), KUnitTerm(TimeUnit.SECOND, -1)))
    ...
    fun `hasSameUnits is false for different exponents`() {
        val a = KUnitInstance(5.0, listOf(KUnitTerm(LengthUnit.METER, 1)))
        val b = KUnitInstance(5.0, listOf(KUnitTerm(LengthUnit.METER, 2)))
    ...
    fun `valueAs converts speed to km per hour using mixed targets in any order`() {
        // 10 m / 1 s
        val speedInstance = KUnitInstance(10.0, listOf(KUnitTerm(LengthUnit.METER, 1), KUnitTerm(TimeUnit.SECOND, -1)))

        val kmh1 = speedInstance.valueAs(KUnitPrefix.KILO with LengthUnit.METER, TimeUnit.HOUR)
        val kmh2 = speedInstance.valueAs(TimeUnit.HOUR, KUnitPrefix.KILO with LengthUnit.METER)
    ...
    fun `valueAs throws on target count mismatch`() {
        ...
        assertFailsWith<IllegalStateException> { speedInstance.valueAs(LengthUnit.METER) }
    }
    ...
    fun `valueAs throws on group mismatch`() {
        ...
        assertFailsWith<IllegalStateException> { speedInstance.valueAs(LengthUnit.METER, LengthUnit.MILE) }
    }
    ...
    fun `valueAs supports derived units for area`() {
        ...
        assertEquals(1.0, area.valueAs(LengthDerivedUnit.HECTARE), 1e-9)
    }

    @Test
    fun `toString without targets uses raw units`() {
        val instance = KUnitInstance(5.0, listOf(KUnitTerm(LengthUnit.METER, 1), KUnitTerm(TimeUnit.SECOND, -1)))
    ...
    fun `toString with targets uses converted units`() {
        ...
        assertEquals("36.0 km*h^-1", speedInstance.toString(KUnitPrefix.KILO with LengthUnit.METER, TimeUnit.HOUR))
    }
}
