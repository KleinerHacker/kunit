...
class KMolarConductivityPrefixTest {
    ...
    fun `si prefix on siemens square meters per mole`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.siemensSquareMetersPerMole).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on siemens square centimeters per mole`(builder: KPrefixBuilder, factor: Double) {
