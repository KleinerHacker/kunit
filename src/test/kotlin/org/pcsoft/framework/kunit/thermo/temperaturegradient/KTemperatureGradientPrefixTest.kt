...
class KTemperatureGradientPrefixTest {
    ...
    fun `si prefix on kelvin per meter`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.kelvinPerMeter).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on kelvin per kilometer`(builder: KPrefixBuilder, factor: Double) {
    ...
    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on fahrenheit per foot`(builder: KPrefixBuilder, factor: Double) {
