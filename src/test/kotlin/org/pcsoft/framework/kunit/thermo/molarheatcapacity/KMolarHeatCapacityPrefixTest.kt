...
class KMolarHeatCapacityPrefixTest {
    ...
    fun `si prefix on joules per mole kelvin`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.joulesPerMoleKelvin).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on calories per mole kelvin`(builder: KPrefixBuilder, factor: Double) {
