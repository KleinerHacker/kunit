...
class KSpecificEnergyPrefixTest {
    ...
    fun `si prefix on joules per kilogram`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.joulesPerKilogram).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on calories per gram`(builder: KPrefixBuilder, factor: Double) {
    ...
    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on watt hours per kilogram`(builder: KPrefixBuilder, factor: Double) {
    ...
    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on btus per pound`(builder: KPrefixBuilder, factor: Double) {
