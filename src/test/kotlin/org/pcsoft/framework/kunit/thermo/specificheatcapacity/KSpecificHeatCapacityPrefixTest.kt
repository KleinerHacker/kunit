...
class KSpecificHeatCapacityPrefixTest {
    ...
    fun `si prefix on joules per kilogram kelvin`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.joulesPerKilogramKelvin).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on calories per gram kelvin`(builder: KPrefixBuilder, factor: Double) {
    ...
    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on btus per pound fahrenheit`(builder: KPrefixBuilder, factor: Double) {
