...
class KThermalConductivityPrefixTest {
    ...
    fun `si prefix on watts per meter kelvin`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.wattsPerMeterKelvin).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on btus per hour foot fahrenheit`(builder: KPrefixBuilder, factor: Double) {
    ...
    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on calories per second centimeter kelvin`(builder: KPrefixBuilder, factor: Double) {
