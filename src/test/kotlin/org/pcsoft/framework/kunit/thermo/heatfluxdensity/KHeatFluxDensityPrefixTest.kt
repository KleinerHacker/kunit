...
class KHeatFluxDensityPrefixTest {
    ...
    fun `si prefix on watts per square meter`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.wattsPerSquareMeter).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on btus per hour square foot`(builder: KPrefixBuilder, factor: Double) {
    ...
    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on calories per second square centimeter`(builder: KPrefixBuilder, factor: Double) {
