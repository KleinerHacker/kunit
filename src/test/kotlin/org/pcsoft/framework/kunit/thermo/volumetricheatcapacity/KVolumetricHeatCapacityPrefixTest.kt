...
class KVolumetricHeatCapacityPrefixTest {
    ...
    fun `si prefix on joules per cubic meter kelvin`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.joulesPerCubicMeterKelvin).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on calories per cubic centimeter kelvin`(builder: KPrefixBuilder, factor: Double) {
