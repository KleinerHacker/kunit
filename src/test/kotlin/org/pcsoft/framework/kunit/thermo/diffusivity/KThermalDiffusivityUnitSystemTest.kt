...
/** `KThermalDiffusivityUnitInstance` surface: round-trip, equality, `toString`, operators, conversion. */
class KThermalDiffusivityUnitSystemTest {
    ...
    @Test
    fun `toThermalDiffusivity round-trip and failure`() {
        val m2 = (1 of meters).toUnit() pow 2
        val s = (1 of seconds).toUnit()
        assertEquals(1.0, (m2 / s).toThermalDiffusivity() into squareMetersPerSecond, 1e-9)

        assertFailsWith<IllegalStateException> { m2.toThermalDiffusivity() }
        assertFailsWith<IllegalStateException> {
            (((1 of meters).toUnit() pow 3) / s).toThermalDiffusivity()
        }
        assertFailsWith<IllegalStateException> { (m2 / (s pow 2)).toThermalDiffusivity() }
    }
}
