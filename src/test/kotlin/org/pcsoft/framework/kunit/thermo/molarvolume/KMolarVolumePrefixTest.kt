...
class KMolarVolumePrefixTest {
    ...
    fun `si prefix on cubic meters per mole`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.cubicMetersPerMole).value, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on liters per mole`(builder: KPrefixBuilder, factor: Double) {
    ...
    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on cubic centimeters per mole`(builder: KPrefixBuilder, factor: Double) {
