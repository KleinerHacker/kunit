...
/** `KThermalInsulanceUnitInstance` surface: round-trip, equality, `toString`, operators, conversion. */
class KThermalInsulanceUnitSystemTest {
    ...
    @Test
    fun `toThermalInsulance round-trip and failure`() {
        val kelvin = KTemperatureDifference.ofKelvin(1).toUnit()
        val g = (1000 of grams).toUnit()
        val s3 = (1 of seconds).toUnit() pow 3
        assertEquals(
            1.0,
            (s3 * kelvin / g).toThermalInsulance() into squareMeterKelvinPerWatt,
            1e-9,
        )

        assertFailsWith<IllegalStateException> { g.toThermalInsulance() }
        assertFailsWith<IllegalStateException> { (s3 * kelvin / (g pow 2)).toThermalInsulance() }
        assertFailsWith<IllegalStateException> {
            (((1 of seconds).toUnit() pow 2) * kelvin / g).toThermalInsulance()
        }
        assertFailsWith<IllegalStateException> { (s3 * (kelvin pow 2) / g).toThermalInsulance() }
    }
}
