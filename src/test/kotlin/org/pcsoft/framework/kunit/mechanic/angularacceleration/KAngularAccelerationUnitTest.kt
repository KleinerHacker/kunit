...
class KAngularAccelerationUnitTest {
    ...
    fun `named tokens in radians per second squared`() {
        ...
        assertEquals(Math.PI / 180.0, (1 of degreesPerSecondSquared).value, 1e-12)
        assertEquals(2.0 * Math.PI, (1 of revolutionsPerSecondSquared).value, 1e-12)
        assertEquals(2.0 * Math.PI / 60.0, (1 of revolutionsPerMinutePerSecond).value, 1e-12)
    }
    ...
    fun `enum entries expose symbol and factor`() {
        ...
        assertEquals(Math.PI / 180.0, KAngularAccelerationUnit.DEGREES_PER_SECOND_SQUARED.baseValue, 1e-12)
        assertEquals(2.0 * Math.PI, KAngularAccelerationUnit.REVOLUTIONS_PER_SECOND_SQUARED.baseValue, 1e-12)
        assertEquals(
            2.0 * Math.PI / 60.0,
    ...
    fun `conversions between tokens`() {
        ...
        assertEquals(180.0 / Math.PI, (1 of radiansPerSecondSquared) into degreesPerSecondSquared, 1e-9)
    }
