...
class KAcousticImpedancePrefixTest {
    ...
    fun `si prefix on pascal seconds per meter`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(
            factor,
    ...
    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on rayls`(builder: KPrefixBuilder, factor: Double) {
        assertEquals(factor, (1 of builder.rayls) into pascalSecondsPerMeter, rel(factor))
    }

    @ParameterizedTest
    @MethodSource("prefixArgs")
    fun `si prefix on cgs rayls`(builder: KPrefixBuilder, factor: Double) {
