...
class KCookbookTest {
    ...
    fun `circumferential speed`() {
        val v = (Math.PI * 0.7 of meters) * (900 of rpm)
        assertIs<KSpeedUnitInstance>(v)
        assertEquals(Math.PI * 0.7 * 15.0, v into meters / seconds, 1e-9)
    }
    ...
    fun `angular motion`() {
        ...
        assertEquals(2.0 * Math.PI, w into radians / seconds, 1e-9)
        ...
        assertEquals(100.0 * Math.PI / 5.0, alpha into radiansPerSecondSquared, 1e-9)
    ...
    fun `inertia and angular momentum`() {
        ...
        assertEquals(0.18 * 100.0 * Math.PI, l into kilogramMetersSquaredPerSecond, 1e-9)

        val inertia = (56 of kilogramMetersSquaredPerSecond) / (50 of revolutionsPerSecond)
        assertIs<KInertiaUnitInstance>(inertia)
        assertEquals(56.0 / (100.0 * Math.PI), inertia into kilogramMetersSquared, 1e-9)
    ...
    fun `torque formulas`() {
        ...
        assertEquals(15_000.0 / (100.0 * Math.PI), fromPower into joules, 1e-9)

        val power = (48 of joules) * (3000 of revolutionsPerMinute)
        assertIs<KPowerUnitInstance>(power)
        assertEquals(48.0 * 100.0 * Math.PI, power into watts, 1e-6)
    ...
    fun `angles and solid angles`() {
        ...
        assertEquals(0.25 / ((Math.PI / 180.0) * (Math.PI / 180.0)), (0.25 of steradians) into squareDegrees, 1e-6)
    }
    ...
    fun `string rendering`() {
        ...
        val v = (120 of kilo.meters) / (1.5 of hours)
        assertEquals("80.0 km/h", "${v into kilo.meters / hours} km/h")
    }
    ...
    fun `worked example motor`() {
        ...
        assertEquals(15_000.0 / (100.0 * Math.PI), torque into joules, 1e-9)
