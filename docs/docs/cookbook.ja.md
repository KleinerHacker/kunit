...
!!! warning "オペランドとしての `pow`"
`pow` は open な `KDistanceUnitInstance` に宣言されているため、`(2 of meters) pow 2` は **実行時には面積** ですが、
静的には距離にすぎません。値を*読み取る*には十分です（`into (meters pow 2)`）が、`pressure * area` のような
型付き演算子には静的に型付けされた面積が必要です。その場合は型付きの長さ演算子から構築してください:
`(0.2 of meters) * (0.1 of meters)` は `KAreaUnitInstance` であり、`liters` はそのまま使える
`KVolumeUnitInstance` です。
