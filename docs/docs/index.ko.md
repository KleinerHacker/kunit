...
- **`KUnitInstance`** - *혼합 단위*(Mischeinheit): `Double` 기본값과 하나 이상의 `KUnit` 조합, 각각 정수
...
`KUnitInstance`를 캡슐화하고 항상 해당 그룹의 기본 단위로 정규화됩니다. 대부분의 경우 이 타입을 사용하게 됩니다 - 현재
제공되는 단위는 [미리 정의된 단위](units/length.md)를, 범용 `KUnitInstance` 엔진을 직접 사용해야 하는 경우는
...
// 순수 단위의 곱셈/나눗셈은 혼합 단위(KUnitInstance)를 생성
