...
`KLatexUnitFormatter` renders a value as **LaTeX** math, ready for MathJax, KaTeX or a LaTeX document. With the default
configuration `3 of meters / seconds` read into `km/h` becomes
...
The layout follows the shared formatter rules: with the `FRACTION` style a clean single-denominator shape (some
numerator, exactly one denominator) is stacked as `\frac{…}{…}`; every other shape — and the whole of the `INLINE`
style — is a flat product joined by the multiplication marker with signed exponents. A dimensionless value renders as
just the number.
