...
يعرض `KAsciiMathUnitFormatter` القيمة بصيغة **AsciiMath**، وهي صيغة الإدخال المختصرة لـ MathJax. مع الإعداد الافتراضي
تصبح `3 of meters / seconds` المقروءة كـ `km/h` هي `10.8 "km"/"h"`.

يوجد في الحزمة `org.pcsoft.framework.kunit.formatter` وهو `class` غير قابل للتغيير وآمن للخيوط.

## ما يُنتجه

نمط `FRACTION` يستخدم صيغة الكسر `a/b` للشكل النظيف ذي المقام الواحد (مع تجميع البسط أو المقام المرفوع إلى أُس بين قوسين
عند الحاجة)؛ وكل شكل آخر — وكامل نمط `EXPONENT` — هو جداء مسطّح مربوط بعلامة الضرب مع أُسٍّ ذات إشارة. القيمة عديمة
الأبعاد تُعرض كرقم فقط.
