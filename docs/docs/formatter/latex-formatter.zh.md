...
`KLatexUnitFormatter` 将数值渲染为 **LaTeX** 数学式，可直接用于 MathJax、KaTeX 或 LaTeX 文档。 默认配置下，
`3 of meters / seconds` 读作 `km/h` 时会得到 `1.5\,\frac{\mathrm{km}}{\mathrm{h}}`。

它位于 `org.pcsoft.framework.kunit.formatter` 包中，是不可变且线程安全的 `class`。

## 生成内容

布局遵循通用规则：在 `FRACTION` 样式下，具有分子且恰好一个分母的规整形式会堆叠为 `\frac{…}{…}`； 其余形式（以及整个 `INLINE`
