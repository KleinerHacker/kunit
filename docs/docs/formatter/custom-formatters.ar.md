...
`KDefaultUnitFormatter` المُرفق نصًا عاديًا مثل `"10.8 km/h"`، لكن يمكنك إدخال عرض مخصص تمامًا — مثل إخراج **LaTeX** أو
**MathML** لعارض صيغ رسومي، أو HTML، أو أي ترميز خاص بمجال. هذا يجعل توسيع kunit نحو أطر خارجية تحوّل السلسلة إلى صيغة
منسّقة أمرًا سهلًا.
...
- أبقِ المنسّق **بلا حالة** ومن ثمّ آمنًا للخيوط — فـ `KDefaultUnitFormatter` المُرفق كائن `object` عادي، وكذلك
  `LatexFormatter` أعلاه.
- يستقبل `KUnitFormatContext` القيمة **مُحوَّلة بالفعل** إلى الوحدة الهدف، لذا لا يجري المنسّق تحويل وحدات بنفسه — بل
  يعرض فقط.
- تحمل حدود `units` بيانات عرض (`KUnitTerm.display`)؛ اقرأ الرمز دائمًا عبر `displaySymbol` حتى تُعرض الوحدات ذات
  البادئات والبديلة (`km`، `mi`، `KiB`) بشكل صحيح.
