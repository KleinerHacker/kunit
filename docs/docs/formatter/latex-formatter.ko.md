...
`KLatexUnitFormatter` 는 값을 **LaTeX** 수식으로 렌더링하여 MathJax, KaTeX 또는 LaTeX 문서에 바로 사용할 수 있습니다. 기본 구성에서
`3 of meters / seconds` 를 `km/h` 로 읽으면 `1.5\,\frac{\mathrm{km}}{\mathrm{h}}` 가 됩니다.
...
`\frac{…}{…}` 로 쌓습니다. 그 외의 형태 (및 `INLINE` 스타일 전체)는 곱셈 기호로 연결한 평면 곱을 부호 있는 지수로 표시합니다. 무차원 값은 숫자만 렌더링합니다.
