...
!!! note "包名与类名的区别"
    包名是 `thermo.conductivity`，而非 `thermo.thermalconductivity` ——
    单位包不得重复其所属领域包的名称。**类型**保留完整的技术术语
    （`KThermalConductivityUnitInstance`），这正是它与
...
一层 30 cm 厚的矿棉（λ = 0.04 W/(m·K)）将 21 °C 的室内与 −5 °C 的室外空气隔开。
墙面 12 m²。热损失是多少？
