...
`KTemperatureGradientUnitInstance` wraps a `KMixedUnitInstance` of exactly two terms in the canonical normal form
`temperature¹ · distance⁻¹` (`K·m⁻¹`), always normalized to K/m.

!!! note "A gradient is a *change* per length"
The temperature dimension is the **difference** group (`KTemperatureDifferenceUnit`). An absolute scale with an offset
(°C, °F) has no meaning in a gradient — only intervals do. That is also why
...
Earth's crust warms by about 25 K per kilometre of depth. A borehole reaches 3.5 km. How much warmer is the rock at the
bottom, and how deep must one drill for a 100 K rise?
