...
Thermal insulance — the **R-value** — is how strongly a layer of a given thickness resists heat flow, per unit of
area: `m²·K/W`. It is the exact reciprocal of the [heat transfer coefficient](heat-transfer-coefficient.md)
(U-value), and the form insulation products are actually sold in, because R-values of layers in series simply
**add up**.

!!! warning "Not the same as thermal resistance"
    Do not confuse this group with the [absolute thermal resistance](thermal-resistance.md) `K/W`, which describes a
    *whole component* rather than a unit of area. The two differ by a factor of area and therefore have different
    normal forms and different types. Up to and including version 0.8.0 this group was named
    `thermo.resistance` / `KThermalResistanceUnit`; it was renamed so the name `thermal resistance` could go to the
    quantity that actually carries it.

`KThermalInsulanceUnitInstance` wraps a `KMixedUnitInstance` of exactly three terms in the canonical normal form
...
name. The **types** keep the full technical term (`KThermalInsulanceUnitInstance`), which distinguishes them from
`electric.resistance`.
...
|---------------------------------------------|----------------------------------------|-----------------------------|
| `temperatureDifference / heatFluxDensity`   | `KThermalInsulanceUnitInstance`       | R from measurement          |
| `length / thermalConductivity`              | `KThermalInsulanceUnitInstance`       | R from material + thickness |
...
| `1 / heatTransferCoefficient`               | `KThermalInsulanceUnitInstance`       | R from U                    |
...
|-------------------------------------------|----------------------------------|----------------------------------|
| `temperatureDifference / heatFluxDensity` | typed operator                   | `KThermalInsulanceUnitInstance` |
| `length / thermalConductivity`            | typed operator                   | `KThermalInsulanceUnitInstance` |
| `mass⁻¹ · time³ · temperature¹`           | native + `toThermalInsulance()` | `KThermalInsulanceUnitInstance` |
