...
`+1`، و`KDistanceUnit.BASE` (متر) بالأس `+2`، و`KTimeUnit.BASE` (ثانية) بالأس `-2`. ولأن مكوّن الكتلة في المكتبة مطبَّع
إلى **غرامات** (وليس كيلوغرامات)، يُقسَم الناتج القياسي على 1000 للوصول إلى الجول؛ وتُطبَّع القيمة المخزّنة دائمًا إلى
الجول.

الطاقة من الناحية التقنية كمّية **واحدة** تظهر في عدة مجالات موضوعية. تصف هذه الصفحة قراءتها *الديناميكية الحرارية* —
**الحرارة**، `Q = Φ · t`. المجموعة نفسها في Kotlin موثّقة للمجالات الأخرى في
[الطاقة (كهربائية)](../electrical/energy.md) و[الطاقة (ميكانيكا)](../mechanics/energy.md).

وعند القسمة على فرق درجة حرارة تصبح الطاقة [سعة حرارية](heat-capacity.md) (أو، عند قراءتها كـ
`ΔS = Q / T`، [إنتروبيا](entropy.md))؛ ولكل كيلوغرام تصبح [طاقة نوعية](specific-energy.md)، ولكل مول
[طاقة مولية](molar-energy.md).
...
تدعم الوحدات المسمّاة بادئات النظام الدولي عبر `KPrefixBuilder` (`kilo.calories` — "سعرة الطعام" — و`kilo.joules`، و
`mega.joules`، …).
