...
열역학은 **열과 온도**의 물리학입니다. KUnit에서 이 분야는 온도를 중심으로 하며, 온도는 **관련된
...
두 항목은 의도적으로 자체 타입을 갖는 대신 기존 타입을 **공유**합니다: 엔트로피는 열용량과 차원적으로
동일하며, 열류는 전력과 동일합니다. 정규 기저 차원 형식은 정확히 하나의 타입에 매핑되어야 합니다.
그렇지 않으면 `toX()` 형식 인식이 모호해집니다 — 그 이유는 [엔트로피](entropy.md)와
...
파생 그룹들은 물질 속성에서 총 열 손실에 이르기까지 하나의 연결된 사슬을 형성합니다. 각 단계는 타입이
지정된 연산자이므로, 원시 `KMixedUnitInstance`는 결코 등장하지 않습니다:
...
`diffusivityWith` / `conductivityWith`로 노출됩니다. 왜냐하면 중간에 나오는 체적 열용량 `ρ · c_p`는
모델링된 단위가 아니기 때문입니다.
