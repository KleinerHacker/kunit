...
전력은 기술적으로 여러 주제 영역에 걸쳐 나타나는 **하나의** 물리량입니다. 이 페이지는 그 *열역학적* 해석
— **열유량(heat flow rate)** `Φ = Q / t`, 즉 시간당 열에너지를 설명합니다. 동일한 Kotlin 그룹은 다른
영역에서도 [전력(전기)](../electrical/power.md)와 [전력(역학)](../mechanics/power.md)에서
문서화되어 있습니다.
