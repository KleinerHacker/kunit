...
전력은 기술적으로 여러 주제 영역에 걸쳐 나타나는 **하나의** 물리량입니다. 이 페이지는 그 *열역학적* 해석 — **열유량 (heat flow rate)** `Φ = Q / t`, 즉 시간당 열에너지를
설명합니다. 동일한 Kotlin 그룹은 다른 영역에서도 [전력 (전기)](../electrical/power.md)와 [전력 (역학)](../mechanics/power.md)에서 문서화되어 있습니다.

열역학 내에서 동일한 타입은 **열류 (heat flow)** 해석도 함께 담고 있습니다 — 그 관점과
[열유속밀도](heat-flux-density.md)로 이어지는 사슬은 [열류](heat-flow.md)를 참조하세요.
...
| 표현식               | 결과 타입            | 의미                                                              |
|----------------------|----------------------|-------------------------------------------------------------------|
| `energy / time`      | `KPowerUnitInstance` | 열유량 `Φ = Q / t` ([에너지(열역학)](energy.md) 참조)             |
| `voltage * current`  | `KPowerUnitInstance` | 전기 전력 `P = U · I` ([전력(전기)](../electrical/power.md) 참조) |
| `force * speed`      | `KPowerUnitInstance` | 기계 전력 `P = F · v` ([전력(역학)](../mechanics/power.md) 참조)  |
...
열유량 형태의 역연산자는 에너지, 시간, 전력을 함께 묶습니다:

| 표현식           | 결과 타입             | 의미                               |
|------------------|-----------------------|------------------------------------|
| `power * time`   | `KEnergyUnitInstance` | 전달된 열, `Q = Φ · t` (교환 가능) |
| `energy / power` | `KTimeUnitInstance`   | 필요한 시간, `t = Q / Φ`           |
...
| 수학        | Kotlin                                            | 의미                                            |
|-------------|---------------------------------------------------|-------------------------------------------------|
| `W`         | `watts`                                           | 전력(열유량), 기본 단위(이름이 붙은 토큰, 와트) |
| `Q / t`     | `(1200 of kilo.joules) / (10 of minutes)`         | 열과 시간으로부터 계산된 열유량                 |
