...
热流密度是单位面积上的热流：`power / area`（`W/m²`）。同一个单位也用于度量
*辐照度*和*辐射出射度* —— 即照射到或离开某表面的辐射强度。

`KHeatFluxDensityUnitInstance` 包装了一个恰好由两项组成的 `KMixedUnitInstance`，
处于规范正规形式 `mass¹ · time⁻³`（`kg·s⁻³`），始终以 W/m² 归一化。
...
某屋顶在晴天接收 800 W/m² 的辐照度。阵列覆盖 25 m²，将 20% 的入射辐射转化为电能。
它能输出多少电功率？
