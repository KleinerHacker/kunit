...
美国的"R-30"保温层是 `30 of hourSquareFootFahrenheitPerBtu` ≈ 5.28 m²·K/W。 一套西装大约是 1 clo；羽绒被以托格计。所有单位均支持完整的
SI 前缀范围。

## 现实示例：分层的保温墙体

一堵墙由 20 cm 矿棉（λ = 0.04 W/ (m·K)）和 12 cm 砖块（λ = 0.8 W/ (m·K)）组成。 总 R 值、由此得到的 U 值以及在 ΔT = 25 K
下的热损失分别是多少？
