...
[熱流束密度](heat-flux-density.md)は、その伝導率に[温度勾配](temperature-gradient.md)を掛けたものに 等しくなります。単位:
`W/(m·K)`。
...
すべてがSI接頭辞の全範囲をサポートします — 断熱材は自然に `40 of milli.wattsPerMeterKelvin` のように 記述されます。
...
30 cmのミネラルウール層 (λ = 0.04 W/ (m·K))が、21 °Cの室内と −5 °Cの屋外空気を隔てています。壁は 12 m²です。どれだけの熱が失われるでしょうか?
