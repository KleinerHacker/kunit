...
패키지: `org.pcsoft.framework.kunit.thermo.diffusivity`
기본 단위: **초당 제곱미터** (`KThermalDiffusivityUnit.BASE == KThermalDiffusivityUnit.SQUARE_METER_PER_SECOND`)

유형: **구성된 단위**

열확산율 `α`는 온도 변화가 물질을 통해 얼마나 *빠르게* 전파되는지를 나타냅니다 — 이는 정상 상태에서
열이 얼마나 *많이* 흐르는지를 나타내는 [열전도율](thermal-conductivity.md)과 대조됩니다. 단위:
`m²/s`. 다음과 같이 정의됩니다:
...
`KThermalDiffusivityUnitInstance`는 정규 형식 `distance² · time⁻¹` (`m²·s⁻¹`)의 정확히 두 항으로
이루어진 `KMixedUnitInstance`를 감싸며, 항상 m²/s로 정규화됩니다.

!!! note "패키지 이름 대 클래스 이름"
    패키지는 `thermo.diffusivity`이며, `thermo.thermaldiffusivity`가 아닙니다 — 단위 패키지는 그 분야
    패키지의 이름을 반복해서는 안 됩니다. 타입은 전체 기술 용어(`KThermalDiffusivityUnitInstance`)를
    유지합니다. 차원 `m²/s`는 동점도 및 물질 확산율과 공유되며, 이 그룹은 열 관련 양을 모델링합니다.
...
물질 표에는 보통 `α`를 mm²/s로 나타내며, 이는 정확히 `micro.squareMetersPerSecond`입니다. 모든 단위는
전체 SI 접두사 범위를 지원합니다.
...
정의 관계는 **삼항**(`α = λ / (ρ · c_p)`)이므로, 여기 있는 다른 모든 그룹과 달리 이 라이브러리가
모델링하지 않는 체적 열용량 `ρ · c_p`(J/(m³·K))에 대한 중간 타입을 새로 만들지 않는 한 단일 이항
연산자가 될 수 없습니다. 따라서 이 관계는 이름이 붙은, 강한 타입의 함수로 노출됩니다:
...
| 분해 | 형식 | 결과 |
|---|---|---|
| `λ / (ρ · c_p)` | 타입이 지정된 함수 `diffusivityWith` | `KThermalDiffusivityUnitInstance` |
| `distance² · time⁻¹` | 네이티브 표현식 + `toThermalDiffusivity()` | `KThermalDiffusivityUnitInstance` |
