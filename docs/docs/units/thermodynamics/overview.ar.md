...
مدخلان يُشاركان عمدًا نوعًا موجودًا بدلًا من امتلاك نوع خاصّ بهما: الإنتروبيا مطابقة بُعديًا للسعة
الحرارية، وتدفّق الحرارة مطابق للقدرة. يجب أن تُطابِق الصيغة القياسية بالأبعاد الأساسية نوعًا واحدًا
بالضبط، وإلّا لأصبح تمييز الصيغة عبر `toX()` مُلتبِسًا — راجع صفحتَي
...
تُشكِّل المجموعات المشتقّة سلسلة واحدة متّصلة من خاصّية المادّة إلى إجمالي الفقد الحراري. كل خطوة معامل
محكوم بالنوع، بحيث لا يظهر أبدًا `KMixedUnitInstance` خام:
