...
Base unit: **joule per kilogram-kelvin** (`KSpecificHeatCapacityUnit.BASE == KSpecificHeatCapacityUnit.JOULE_PER_KILOGRAM_KELVIN`)
...
`KSpecificHeatCapacityUnitInstance` wraps a `KMixedUnitInstance` of exactly three terms in the canonical
normal form `distance² · time⁻² · temperature⁻¹` (`m²·s⁻²·K⁻¹`) — the mass dimension cancels out, exactly
as for [specific energy](specific-energy.md). The temperature dimension is the **difference** group
...
150 liters of water (150 kg) are heated from 12 °C to 40 °C. Water has a specific heat capacity of
4184 J/(kg·K).
