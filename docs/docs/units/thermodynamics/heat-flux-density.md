...
Heat flux density is heat flow per unit of area: `power / area` (`W/m²`). The same unit measures
*irradiance* and *radiant exitance* — the intensity of radiation hitting or leaving a surface.

`KHeatFluxDensityUnitInstance` wraps a `KMixedUnitInstance` of exactly two terms in the canonical normal
form `mass¹ · time⁻³` (`kg·s⁻³`), always normalized to W/m².
...
A rooftop receives 800 W/m² on a clear day. The array covers 25 m² and converts 20 % of the incident
radiation. What electrical power does it deliver?
