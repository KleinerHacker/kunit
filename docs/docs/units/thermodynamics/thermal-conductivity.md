...
`KThermalConductivityUnitInstance` wraps a `KMixedUnitInstance` of exactly four terms in the canonical normal form
`mass¹ · distance¹ · time⁻³ · temperature⁻¹` (`kg·m·s⁻³·K⁻¹`), always normalized to W/ (m·K).

!!! note "Package name vs. class name"
The package is `thermo.conductivity`, not `thermo.thermalconductivity` — a unit package must not repeat its field
package's name. The **types** keep the full technical term (`KThermalConductivityUnitInstance`), which is what
...
A 30 cm mineral-wool layer (λ = 0.04 W/ (m·K)) separates a 21 °C room from −5 °C outside air. The wall is 12 m². How
much heat is lost?
