...
열유속밀도는 단위 면적당 열류입니다: `power / area` (`W/m²`). 동일한 단위가 *복사조도(irradiance)*와
*복사발산도(radiant exitance)* — 표면에 부딪히거나 떠나는 복사의 강도 — 도 측정합니다.
...
총 열류 자체는 단순히 [전력](power.md)입니다. [열류](heat-flow.md)를 참조하세요. 온도 차로 나누면
이는 [열전달계수](heat-transfer-coefficient.md)가 됩니다.
...
맑은 날 지붕이 800 W/m²를 받습니다. 어레이는 25 m²를 덮고 입사 복사의 20 %를 변환합니다. 얼마의
전기 전력을 공급할까요?
