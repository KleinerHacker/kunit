...
温度由 **两个相关的组**建模,因为温度读数与温度 *变化*在物理上是不同种类的量。正确处理这一区别,正是让运算 保持正确的关键。
...
**绝对温度**(`25 °C`、`300 K`)是温度标度上的仿射 *点*:它从固定的零点测量,零点的选择 (0 K 还是 0 °C)会改变 数值。
**温度差**(`20 ΔK`)是 *向量* —— 两点之间的间隔。它没有零点包袱: `20 ΔK` 的差无论位于标度何处都等于
`20 °C` 的差。
...
* **作为差值 / 变化** → 使用 **温度差**。零点相互抵消,因此这里 `°C` 与 `K` 可以互换。典型案例 —— 热能:
  ...
  `30 °C − 10 °C = 20 ΔK`,且 `Q = m·c·20 ΔK`。热膨胀 (`ΔL = α·L·ΔT`)、热传导,以及比热容 / 热导率单位 (`J/(kg·ΔK)`、
  `W/(m·ΔK)`)同理。
...
两者使用不同的单位组,因此包含绝对开尔文的混合单位与包含差值开尔文的混合单位既不相等也不可相加。区分的 **`ΔK`** 符号 (相对
`K`)正是为了在 `toString` 和混合单位输出中让这一点可见 —— 在构造自己的混合单位时请 牢记这一点。
