...
[热导率](thermal-conductivity.md)不同，后者描述的是稳态下 *流过多少*热量。 单位：`m²/s`。其定义为

```
α = λ / (ρ · c_p)
```

`KDiffusivityUnitInstance` 包装了一个恰好由两项组成的
`KMixedUnitInstance`，处于规范正规形式 `distance² · time⁻¹`（`m²·s⁻¹`）， 始终以 m²/s 归一化。

!!! note "一个组，两个学科领域"
该组位于 `common.diffusivity`，因为完全相同的物理量在两个领域中都有文档： 在此处作为 **热扩散率**，在力学中作为
[运动粘度](../mechanics/kinematic-viscosity.md) `ν = η / ρ`。两种读法共享
`KDiffusivityUnit` 的单位词汇表，包括传统的斯托克斯写法。
...
材料手册中通常以 mm²/s 列出 `α`，这恰好等于 `micro.squareMetersPerSecond`。 所有单位均支持完整的 SI 前缀范围。
...
其定义关系是 **三元**的（`α = λ / (ρ · c_p)`），因此与本页其他每个组不同， 它无法用单一的二元操作符表示，除非为体积热容
`ρ · c_p`（J/ (m³·K)） 发明一个中间类型，而本库并未建模该类型。因此该关系以命名的、强类型的 函数形式暴露：
...
| 分解方式             | 形式                           | 结果                       |
|----------------------|--------------------------------|----------------------------|
| `λ / (ρ · c_p)`      | 类型化函数 `diffusivityWith`   | `KDiffusivityUnitInstance` |
| `distance² · time⁻¹` | 原生表达式 + `toDiffusivity()` | `KDiffusivityUnitInstance` |
