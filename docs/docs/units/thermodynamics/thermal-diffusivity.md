...
Base unit: **square meter per second** (`KDiffusivityUnit.BASE == KDiffusivityUnit.SQUARE_METER_PER_SECOND`)

Type: **constructed unit**

Thermal diffusivity `α` says how *fast* a temperature change propagates through a material — as opposed
to [thermal conductivity](thermal-conductivity.md), which says how *much* heat flows in the steady state. Unit: `m²/s`.
It is defined as
...
`KDiffusivityUnitInstance` wraps a `KMixedUnitInstance` of exactly two terms in the canonical normal form
`distance² · time⁻¹` (`m²·s⁻¹`), always normalized to m²/s.

!!! note "One group, two subject areas"
The group lives in `common.diffusivity` because the very same quantity is documented in two fields:
as the **thermal diffusivity** here and as the
[kinematic viscosity](../mechanics/kinematic-viscosity.md) `ν = η / ρ` in mechanics. Both readings share the unit
vocabulary of `KDiffusivityUnit`, including the traditional stokes spellings.
...
Material tables list `α` in mm²/s, which is exactly `micro.squareMetersPerSecond`. All units accept the full SI prefix
range.
...
The defining relation is **ternary** (`α = λ / (ρ · c_p)`), so unlike every other group here it cannot be a single
binary operator without inventing an intermediate type for the volumetric heat capacity
`ρ · c_p` (J/ (m³·K)), which this library does not model. The relation is therefore exposed as named, strongly typed
functions:
...
|----------------------|---------------------------------------|----------------------------|
| `λ / (ρ · c_p)`      | typed function `diffusivityWith`      | `KDiffusivityUnitInstance` |
| `distance² · time⁻¹` | native expression + `toDiffusivity()` | `KDiffusivityUnitInstance` |
