...
ऐफ़ाइन अपवाद** है: हर अन्य समूह के विपरीत, तापमान इकाइयों के बीच रूपांतरण कोई एकल गुणक नहीं बल्कि एक **ऑफ़सेट-और-मापन**
(ऐफ़ाइन) परिवर्तन है — `25 °C`, `25 × 1 °C` *नहीं* है। मान **परम केल्विन** में प्रसामान्यीकृत संग्रहित होते हैं, इसलिए
`*`/`/`/`pow` सामान्य इंजन से अपरिवर्तित चलते रहते हैं।

दो बातें इस समूह को विशेष बनाती हैं:

* **हुक के माध्यम से ऐफ़ाइन रूपांतरण, अतिभार के माध्यम से नहीं।** साझा इंजन शुद्ध रूप से गुणात्मक रहता है। ऐफ़ाइन
  परिवर्तन दो मापन-योग्य हुक `scaledBy` (निर्माण, `of` के पीछे) और `readBaseValue` (पठन, `into`
  के पीछे) के माध्यम से इंजेक्ट होता है, इसलिए `25 of celsius` और `t into fahrenheit` सामान्य क्रियाओं के माध्यम से काम
  करते हैं — `of`/`into` के लिए कोई समूह-विशिष्ट अतिभार नहीं (जिसे एक स्पष्ट रूप से आयातित सामान्य क्रिया ढँक देती)।
* **कोई उपसर्ग नहीं।** तापमान समूह जानबूझकर **कोई** उपसर्ग बिल्डर प्रदान नहीं करता (`milli.celsius` मॉडल नहीं किया गया)।
  कोई `KTemperatureUnitExtensions.kt` नहीं है।
...
प्रत्येक `Token` एक मान-1 `KTemperatureUnitInstance` है जो `of` (निर्माण) और `into` (पठन) के साथ प्रयुक्त होता है।
...
एक परम तापमान एक ऐफ़ाइन **बिंदु** है, सदिश नहीं। इसलिए इसका अंकगणित जानबूझकर असममित है — भौतिक रूप से सही व्यवहार
([तापमान अंतर](temperature-difference.md) भी देखें):
...
* `परम * संख्या` / `परम / संख्या` ← **संकलन त्रुटि**: किसी ऐफ़ाइन बिंदु को सादे संख्या से मापना निरर्थक है (इसका केल्विन
  मान −273.15 ऑफ़सेट वहन करता है)। इसके बजाय एक [तापमान अंतर](temperature-difference.md)
  मापें, जो रैखिक है।
...
`==`, `!=`, `<`, `<=`, `>`, `>=` प्रसामान्यीकृत परम केल्विन `value` की तुलना करते हैं। `equals` परम तापमान से होता है,
निर्माण इकाई से स्वतंत्र, इसलिए `(0 of celsius) == (273.15 of kelvin)`।

## `pow` से घात

infix `pow` संकारक से किसी मान को एक पूर्णांक घात तक उठाएँ। तापमान समूह के लिए `pow` एक सामान्य
`KMixedUnitInstance` लौटाता है (तापमान के पास कोई विमायुक्त घात प्रकार नहीं), जो परम केल्विन पद पर रैखिक रूप से कार्य
करता है:
...
किसी तापमान को किसी अन्य समूह से गुणा या भाग करने पर एक सामान्य `KMixedUnitInstance` मिलता है (कोई मानक तापमान संयोजन
नहीं), जो परम केल्विन मान पर गणना किया जाता है:
...
घातांक Unicode उपरिलेख (`²`, `³`, `⁻¹`) से लिखे जाते हैं, `·` गुणन और `/` भिन्न दर्शाता है। तापमान ऐफ़ाइन है, इसलिए कोई
`·`/घातांक गुणनफल रूप नहीं है — केवल नामित इकाइयाँ और ऑफ़सेट परिवर्तन।
