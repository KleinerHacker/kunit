...
`KSpecificHeatCapacityUnitInstance`는 정규 형식 `distance² · time⁻² · temperature⁻¹` (`m²·s⁻²·K⁻¹`)의
정확히 세 항으로 이루어진 `KMixedUnitInstance`를 감쌉니다 — [비에너지](specific-energy.md)와 정확히
동일하게 질량 차원이 상쇄됩니다. 온도 차원은 **차** 그룹(`KTemperatureDifferenceUnit`)이며, 아핀 절대
온도가 아닙니다.
