...
पैकेज: `org.pcsoft.framework.kunit.thermo.diffusivity`
मूल इकाई: **वर्ग मीटर प्रति सेकंड** (`KThermalDiffusivityUnit.BASE == KThermalDiffusivityUnit.SQUARE_METER_PER_SECOND`)

प्रकार: **संघटित इकाई**

तापीय विसरणशीलता `α` बताती है कि तापमान परिवर्तन किसी सामग्री में कितनी *तेजी* से फैलता है — इसके
विपरीत [तापीय चालकता](thermal-conductivity.md) बताती है कि स्थिर अवस्था में *कितनी* ऊष्मा प्रवाहित
होती है। इकाई: `m²/s`। इसे इस प्रकार परिभाषित किया जाता है

```
α = λ / (ρ · c_p)
```

`KThermalDiffusivityUnitInstance` एक `KMixedUnitInstance` को लपेटता है जिसमें विहित सामान्य रूप
`distance² · time⁻¹` (`m²·s⁻¹`) में ठीक दो पद होते हैं, जो हमेशा m²/s में सामान्यीकृत रहता है।

!!! note "पैकेज नाम बनाम क्लास नाम"
    पैकेज `thermo.diffusivity` है, `thermo.thermaldiffusivity` नहीं — किसी यूनिट पैकेज को अपने फील्ड
    पैकेज का नाम दोहराना नहीं चाहिए। टाइप्स पूरा तकनीकी शब्द बनाए रखते हैं
    (`KThermalDiffusivityUnitInstance`)। आयाम `m²/s` गतिज श्यानता (kinematic viscosity) और द्रव्यमान
    विसरणशीलता के साथ साझा किया जाता है; यह समूह तापीय राशि का मॉडल बनाता है।
...
सामग्री तालिकाएँ `α` को mm²/s में सूचीबद्ध करती हैं, जो ठीक `micro.squareMetersPerSecond` है। सभी
इकाइयों में पूर्ण SI उपसर्ग सीमा समर्थित है।
...
परिभाषित संबंध **त्रिआधारी (ternary)** है (`α = λ / (ρ · c_p)`), इसलिए यहाँ के हर दूसरे समूह के
विपरीत यह वॉल्यूमेट्रिक ऊष्मा क्षमता `ρ · c_p` (J/(m³·K)) के लिए एक मध्यवर्ती टाइप बनाए बिना एकल
द्विआधारी संकारक नहीं हो सकता, जिसे यह लाइब्रेरी मॉडल नहीं करती। इसलिए यह संबंध नामित, दृढ़ता से
टाइप किए गए फ़ंक्शनों के रूप में उजागर किया गया है:
...
| अपघटन | रूप | परिणाम |
|---|---|---|
| `λ / (ρ · c_p)` | टाइप किया गया फ़ंक्शन `diffusivityWith` | `KThermalDiffusivityUnitInstance` |
| `distance² · time⁻¹` | मूल व्यंजक + `toThermalDiffusivity()` | `KThermalDiffusivityUnitInstance` |
