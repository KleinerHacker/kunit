...
(`value`/`valueIn`/`+`/`-`/`*`/`/`/`toString`/`toKUnitInstance`), so a time value plugs into the generic
...
no time² or 1/time wrapper (multiplying/dividing "escapes" to a raw `KUnitInstance`, exactly like
length). Consequently `KUnitInstance.toKTimeUnit()` accepts only a single `KTimeUnit` term **at exponent 1**.
...
val ratio = 10.seconds() / 2.seconds()           // KUnitInstance: value=5.0, dimensionless
```
...
// Construction: "5 milli seconds" -> KPrefixBuilder -> KUnitInstance -> KTimeUnitInstance
...
!!! note "Duration range"
    ...
    exceed this range, and sub-nanosecond values round to zero. The generic `KUnitInstance`/prefix layer
