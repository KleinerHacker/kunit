...
`KLengthUnitInstance` 将 `KMixedUnitInstance` 限定为 `KLengthUnit.BASE` 的单个项进行包装，指数可以是任意值——
...
d.valueAs(KLengthUnit.MILE)    // 5.0（换算回英里读取）
...
组合，使用通用的 infix 构造函数以及 `with`（用于 valueAs/toString 目标）：
...
// 构造："5 kilo meters" -> KPrefixBuilder -> KMixedUnitInstance -> KLengthUnitInstance
val fiveKm = (5 kilo meters).toKMixedUnitInstance().toKLengthUnit()
fiveKm.value // 5000.0

// 使用带前缀的目标读取数值
val d = 5.miles()
d.valueAs(KUnitPrefix.KILO with KLengthUnit.METER)  // 8.04672（千米）
d.toString(KUnitPrefix.KILO with KLengthUnit.METER) // "8.04672 km"

// 前缀也可以与派生单位（面积/体积）组合
val tank = 200.liters()
tank.valueAs(KUnitPrefix.MILLI with KLengthDerivedUnit.LITER) // 200000.0（毫升）
```
