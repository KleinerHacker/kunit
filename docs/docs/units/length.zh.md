...
包：`org.pcsoft.framework.kunit.length`
基础单位：**米**（`KLengthUnit.BASE == KLengthUnit.METER`）

`KLengthUnitInstance` 将 `KUnitInstance` 限定为 `KLengthUnit.BASE` 的单个项进行包装，指数可以是任意值——
...
|---|---|---|---:|---:|
| 米 | `KLengthUnit.METER` | `m` | `Number.meters()` | 1.0 |
| 英里 | `KLengthUnit.MILE` | `mi` | `Number.miles()` | 1609.344 |
| 海里 | `KLengthUnit.NAUTICAL_MILE` | `nmi` | `Number.nauticalMiles()` | 1852.0 |
| 码 | `KLengthUnit.YARD` | `yd` | `Number.yards()` | 0.9144 |
| 英尺 | `KLengthUnit.FOOT` | `ft` | `Number.feet()` | 0.3048 |
| 英寸 | `KLengthUnit.INCH` | `in` | `Number.inches()` | 0.0254 |
| 英寻 | `KLengthUnit.FATHOM` | `ftm` | `Number.fathoms()` | 1.8288 |
...
以上每个单位都有对应的裸 `val` 别名，可用作 `valueIn`/`toString` 的目标，或作为前缀 infix 函数的 `unit`
...
d.valueIn(KLengthUnit.MILE)    // 5.0（换算回英里读取）
d.valueIn(feet)                 // 26400.0
d.valueIn(nauticalMiles)        // ≈ 4.3452（换算为海里读取）
```
...
`KLengthUnit.BASE^2`（平方米）表示外，还可以使用以下命名的特殊单位（`KLengthDerivedUnit`）作为
转换/格式化目标：

| 特殊单位 | 枚举值 | 符号 | 创建函数 | 1 单位对应的 m² |
|---|---:|---:|---:|---:|
| 公亩 | `KLengthDerivedUnit.ARE` | `a` | `Number.ares()` | 100.0 |
| 公顷 | `KLengthDerivedUnit.HECTARE` | `ha` | `Number.hectares()` | 10 000.0 |
| 英亩 | `KLengthDerivedUnit.ACRE` | `ac` | `Number.acres()` | 4046.8564224 |
...
plot + computed.toKLengthUnit()                // 允许：两者都是指数 2（面积）
plot + 5.meters()                              // 抛出 IllegalStateException（面积 vs 长度）
```

## 指数 3 - 体积

指数为 3 的 `KLengthUnitInstance` 表示体积。除了原始的 `KLengthUnit.BASE^3`（立方米）表示外，还可以
使用以下命名的特殊单位：

| 特殊单位 | 枚举值 | 符号 | 创建函数 | 1 单位对应的 m³ |
|---|---:|---:|---:|---:|
| 升 | `KLengthDerivedUnit.LITER` | `L` | `Number.liters()` | 0.001 |
| 美制加仑 | `KLengthDerivedUnit.US_GALLON` | `gal (US)` | `Number.usGallons()` | 0.003785411784 |
| 英制加仑 | `KLengthDerivedUnit.IMPERIAL_GALLON` | `gal (UK)` | `Number.imperialGallons()` | 0.00454609 |
| 美制液量盎司 | `KLengthDerivedUnit.US_FLUID_OUNCE` | `fl oz` | `Number.usFluidOunces()` | 2.95735295625e-5 |
| 石油桶 | `KLengthDerivedUnit.OIL_BARREL` | `bbl` | `Number.oilBarrels()` | 0.158987294928 |
...
tank + cube.toKLengthUnit()                        // 允许：两者都是指数 3（体积）
```

## SI 前缀

任何 `KLengthUnit` 都可以与 24 个 SI 前缀（`KUnitPrefix`，根包，从 Quetta/Q 到 Quecto/q）中的任意一个
组合，使用通用的 infix 构造函数以及 `with`（用于 valueIn/toString 目标）：
...
// 构造："5 kilo meters" -> KPrefixBuilder -> KUnitInstance -> KLengthUnitInstance
val fiveKm = (5 kilo meters).toKUnitInstance().toKLengthUnit()
fiveKm.value // 5000.0

// 使用带前缀的目标读取数值
val d = 5.miles()
d.valueIn(KUnitPrefix.KILO with KLengthUnit.METER)  // 8.04672（千米）
d.toString(KUnitPrefix.KILO with KLengthUnit.METER) // "8.04672 km"

// 前缀也可以与派生单位（面积/体积）组合
val tank = 200.liters()
tank.valueIn(KUnitPrefix.MILLI with KLengthDerivedUnit.LITER) // 200000.0（毫升）
```
