...
パッケージ: `org.pcsoft.framework.kunit.capacitance`
...
に正規化され、質量の指数が*負*であるため、ファラドは生の成分基準に対して逆方向に1000倍です。
保存される値はファラドに正規化されます。

## 静電容量の作成

静電容量は名前付きトークンから、または分解(下記参照)から作成します。名前付き単位は値1のトークンとして残ります
(`of`/`into` で使用):
...
`toCapacitance()`(正規形のみを認識し、そうでなければ `IllegalStateException` を投げる)で絞り込みます。
両方の経路は値が等しくなります。
