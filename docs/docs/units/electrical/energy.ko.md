...
에너지는 기술적으로 여러 주제 영역에 걸쳐 나타나는 **하나의** 물리량입니다. 이 페이지는 그 *전기적* 해석 (`W = Q · U`, 그리고 소비된 전기 에너지의 경우 `W = P · t`)을 설명합니다. 동일한
Kotlin 그룹은 다른 영역에서도 [에너지 (역학)](../mechanics/energy.md)와 [에너지 (열역학)](../thermodynamics/energy.md)에서 문서화되어 있습니다.
