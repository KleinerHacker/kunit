...
kilograms), the canonical product is divided by 1000 to reach webers; the stored value is always normalized to webers.

## Building a magnetic flux

Build a flux with a named token, or from a decomposition (see below). Named units survive as value-1 tokens (used with
`of`/`into`):
...
Magnetic flux can be reached through several **equivalent decompositions**, all producing the same value-equal flux:
...
`KMixedUnitInstance` and is narrowed with `toMagneticFlux()` (which recognises only the canonical normal form and throws
