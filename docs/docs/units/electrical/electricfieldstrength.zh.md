...
`KElectricCurrentUnit.BASE`（安培）指数为 `-1`。由于库中的质量分量以 **克**（而非千克）为归一化基准， 规范乘积需除以 1000
才能得到伏特每米；存储值始终以伏特每米归一化。
...
可以用一个命名令牌构建电场强度，也可以通过分解构建（见下文）。命名单位以值为 1 的 令牌形式存在（配合 `of`/`into` 使用）：
...
`KMixedUnitInstance`，需通过 `toElectricFieldStrength()` 收窄为具体类型（该方法仅识别规范正规形式， 否则会抛出
