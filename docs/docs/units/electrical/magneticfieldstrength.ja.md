...
`KMagneticFieldStrengthUnitInstance` は2つの項 — 指数 `+1` の `KElectricCurrentUnit.BASE`(アンペア)と
指数 `-1` の `KDistanceUnit.BASE`(メートル) — をラップします。保存される値は常にアンペア毎メートルに
正規化されます。
...
磁界の強さは名前付きトークンから、または分解(下記参照)から作成します。名前付き単位は値1のトークンとして
残ります(`of`/`into` で使用):
