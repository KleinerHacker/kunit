...
（`kg⁻¹·m⁻²·s²·A²` = `A/Wb` = `H⁻¹`）です。`KReluctanceUnitInstance` は4つの項からなる `KMixedUnitInstance`
をラップします —
...
（アンペア回数で測定、[電流](ec.md)参照）を、ホプキンソンの法則 `Θ = Rm · Φ` を通じて 生じる[磁束](magneticflux.md)
と関係付けます。その逆数は **パーミアンス** `Λ` であり、 ヘンリーで測定されるため[インダクタンス](inductance.md)グループが担います。

## 磁気抵抗を組み立てる

名前付きトークンで、または分解表現（下記参照）から磁気抵抗を組み立てられます。名前付き単位は値1の トークンとして存在します（
`of`/`into` と併用）。
