...
パッケージ: `org.pcsoft.framework.kunit.electric.resistance`
...
抵抗は名前付きトークンから、または分解 (下記参照)から作成します。名前付き単位は値1のトークンとして残ります (`of`/`into`
で使用):
...
`toResistance()`(正規形のみを認識し、そうでなければ `IllegalStateException` を投げる)で絞り込みます。 両方の経路は値が等しくなります。
...
交流システムでは回路の抵抗成分は3つの量に分けられ、いずれも抵抗と **次元的に同一**であり、すべてオーム で測定されます:
...
リアクタンス成分は[インダクタンス](inductance.md)と[静電容量](capacitance.md)によって駆動され、それが運ぶ 電力は `var`
で表されます。[電力（電気）](power.md)を参照してください。
