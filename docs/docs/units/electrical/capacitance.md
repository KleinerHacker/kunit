...
kilograms) and the mass exponent is *negative*, the farad is 1000× the raw component base in the opposite direction; the
stored value is normalized to farads.

## Building a capacitance

Build a capacitance with a named token, or from a decomposition (see below). Named units survive as value-1 tokens (used
with `of`/`into`):
...
Capacitance can be reached through several **equivalent decompositions**, all producing the same value-equal
capacitance:
...
`KMixedUnitInstance` and is narrowed with `toCapacitance()` (which recognises only the canonical normal form and throws
