...
reach farads per meter; the stored value is always normalized to farads per meter.
...
Build a permittivity with a named token, or from a decomposition (see below). Named units survive as value-1
...
Named units support the SI prefixes via `KPrefixBuilder` (`pico.faradsPerMeter`, `nano.faradsPerMeter`, …).
The constant is also available as `KPermittivityUnit.VACUUM_PERMITTIVITY`.
...
`KMixedUnitInstance` and is narrowed with `toPermittivity()` (which recognises only the canonical normal form
