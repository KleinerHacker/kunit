...
`KTimeUnit.BASE`(초)는 `-3`, `KElectricCurrentUnit.BASE`(암페어)는 `-2`입니다. 라이브러리의 질량 구성 요소는 **그램**(킬로그램이 아님)으로 정규화되어 있으므로, 정규
곱은 옴미터에 도달하기 위해 1000으로 나뉩니다; 저장된 값은 항상 옴미터로 정규화됩니다.

저항률은 저항의 배후에 있는 물질 특성이며 [전도율](conductivity.md)의 역수입니다 (`ρ = 1 / σ`).

## 저항률 만들기

이름이 붙은 토큰으로 저항률을 만들거나, 아래의 분해식으로부터 만들 수 있습니다. 이름이 붙은 단위는 값 1의 토큰으로 존재합니다 (`of`/`into`와 함께 사용):
...
`KMixedUnitInstance`로 남아 있으며 `toResistivity()`로 좁혀집니다 (이는 정규 형식만 인식하고 그렇지 않으면 `IllegalStateException`을 발생시킵니다). 모든 경로는
값-동등합니다.
