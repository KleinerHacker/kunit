...
`KElectricCurrentUnit.BASE`(암페어)는 `-1`입니다. 라이브러리의 질량 구성 요소는 **그램**(킬로그램이 아님)으로 정규화되어 있으므로, 정규 곱은 웨버에 도달하기 위해 1000으로
나뉩니다; 저장된 값은 항상 웨버로 정규화됩니다.

## 자기 선속 만들기

이름이 붙은 토큰으로 선속을 만들거나, 아래의 분해식으로부터 만들 수 있습니다. 이름이 붙은 단위는 값 1의 토큰으로 존재합니다 (`of`/`into`와 함께 사용):
