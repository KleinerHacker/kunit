...
`KCapacitanceUnitInstance` 包装一个包含四个项的 `KMixedUnitInstance` — 指数 `-1` 的 `KMassUnit.BASE`(克)、 指数 `-2` 的
`KDistanceUnit.BASE`(米)、指数 `+4` 的 `KTimeUnit.BASE`(秒)以及指数 `+2` 的
`KElectricCurrentUnit.BASE`(安培)。由于本库的质量分量归一化到 **克**(而非千克),且质量指数为 *负*, 法拉相对于原始分量基准在相反方向上相差
1000 倍;存储的值归一化为法拉。
...
下表展示该单位及其组成部分在数学上的写法与在 KUnit 的 Kotlin 中的写法。指数使用 Unicode 上标 (`²`、`⁴`、`⁻¹`),`·` 表示乘法,
`/` 表示分数。当一个量既可写成分数也可写成带负指数的乘积时,两种等价的 Kotlin 形式都会列出。
