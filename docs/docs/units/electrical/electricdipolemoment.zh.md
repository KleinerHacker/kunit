...
包：`org.pcsoft.framework.kunit.electric.electricdipolemoment`
基本单位： **库仑米**
...
可以用一个命名令牌构建电偶极矩，也可以通过分解构建（见下文）。命名单位以值为 1 的 令牌形式存在（配合 `of`/`into` 使用）：
...
德拜在分子物理学和化学中占主导地位。命名单位通过 `KPrefixBuilder` 支持 SI 前缀 （`pico.coulombMeters`、`milli.debyes` 等）。
...
`KMixedUnitInstance`，需通过 `toElectricDipoleMoment()` 收窄为具体类型（该方法仅识别规范正规形式， 否则会抛出
