...
穿过一个闭合曲面的 **电通量** `Ψ` 等于该曲面所包围的电荷（高斯定律,`Ψ = Q`）。因此它与电荷在 **量纲上相同**,同样以库仑为单位。KUnit
用该组和符号 `C` 来表示它;没有单独的令牌,也没有单独的类型:
...
下表展示该单位及其组成部分在数学上的写法与在 KUnit 中的 Kotlin 写法。指数使用 Unicode 上标（`²`、`³`、`⁻¹`）,`·` 表示乘法,`/`
