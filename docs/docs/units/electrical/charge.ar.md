...
الحزمة: `org.pcsoft.framework.kunit.charge`
...
`KTimeUnit.BASE` (ثانية) بالأس `+1`. القيمة المخزّنة تُطبَّع دائمًا إلى الكولوم، بغضّ النظر عن الوحدة
المسمّاة أو البادئة أو تركيبة التيار/الزمن التي أُنشئت منها.
...
عامًّا ويُضيَّق عبر `toCharge()` (الذي يتعرّف فقط على الصيغة القياسية — حدّ `KElectricCurrentUnit` واحد
بالأس `+1` وحدّ `KTimeUnit` واحد بالأس `+1` — ويرمي `IllegalStateException` خلاف ذلك). جميع المسارات
متساوية القيمة.
