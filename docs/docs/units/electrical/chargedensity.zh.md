...
包: `org.pcsoft.framework.kunit.chargedensity`
...
下表展示了该单位及其组成部分在数学写法与 KUnit 的 Kotlin 写法中的对应。指数使用 Unicode 上标（`²`、`³`、`⁻¹`），`·` 表示乘法，`/` 表示分数。对于既可写成分数也可写成含负指数乘积的量，两种等价的 Kotlin 形式都会列出。
