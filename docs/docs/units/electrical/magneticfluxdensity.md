...
Package: `org.pcsoft.framework.kunit.magneticfluxdensity`
...
and `KElectricCurrentUnit.BASE` (ampere) at `-1`. Because the mass component of the library is normalized
to **grams** (not kilograms), the tesla is 1000× the raw component base; the stored value is normalized to
teslas.

## Building a magnetic flux density

Build a flux density with a named token, or from a decomposition (see below). Named units survive as
...
Magnetic flux density can be reached through several **equivalent decompositions**, all producing the same
value-equal flux density:
...
`KMixedUnitInstance` and is narrowed with `toMagneticFluxDensity()` (which recognises only the canonical
