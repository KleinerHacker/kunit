...
kilograms) and the mass exponent is *negative*, the canonical product is multiplied by 1000 to reach siemens per meter;
the stored value is always normalized to S/m.
...
Build a conductivity with a named token, or from a decomposition (see below). Named units survive as value-1 tokens
(used with `of`/`into`):
...
`KMixedUnitInstance` and is narrowed with `toConductivity()` (which recognises only the canonical normal form and throws
