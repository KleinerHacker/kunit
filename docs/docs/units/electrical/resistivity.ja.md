...
パッケージ: `org.pcsoft.framework.kunit.electric.resistivity`
...
（キログラムではない）に正規化されているため、正準積は1000で割ってオーム・メートルに換算されます。
...
名前付きトークンで、または分解表現（下記参照）から抵抗率を組み立てられます。名前付き単位は値1の トークンとして存在します（
`of`/`into` と併用）。
