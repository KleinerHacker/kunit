...
`+3`، و`KElectricCurrentUnit.BASE` (أمبير) عند `+2`. ولأنّ مكوّن الكتلة في المكتبة مُطبَّع إلى **الغرامات** (لا
الكيلوغرامات) ولأنّ أُسّ الكتلة سالب، فإنّ السيمنز يساوي 1/1000 من الأساس الخام للمكوّنات؛ القيمة المخزّنة مُطبَّعة إلى
السيمنز.
...
`siemens` (في هذه الحزمة) هي وحدة **المواصَلة** في النظام الدولي. أمّا `siemensUnits` المشابهة في الاسم داخل
`org.pcsoft.framework.kunit.electric.resistance` فهي **وحدة سيمنز الزئبقية** التاريخية، وهي *مقاومة* مقدارها 0.9534 Ω.
وهما كمّيتان غير مرتبطتين في حزمتين مختلفتين.
