...
`KTimeUnit.BASE`(초)는 `-3`, `KElectricCurrentUnit.BASE`(암페어)는 `-1`입니다. 라이브러리의 질량 구성 요소는 **그램**(킬로그램이 아님)으로 정규화되어 있으므로, 정규
곱은 미터당 볼트에 도달하기 위해 1000으로 나뉩니다; 저장된 값은 항상 미터당 볼트로 정규화됩니다.

장 세기 `E`는 단위 길이당 전압 강하이며, 동시에 단위 전하에 작용하는 힘입니다. 이는
[전속 밀도](electricfluxdensity.md)와 [유전율](permittivity.md)을 통해 연결되며 (`D = ε · E`), 전하 운반자를 [전기 이동도](electricmobility.md)로
주어지는 속도로 움직이게 합니다 (`v = μ · E`).

## 전기장 세기 만들기

이름이 붙은 토큰으로 전기장 세기를 만들거나, 아래의 분해식으로부터 만들 수 있습니다. 이름이 붙은 단위는 값 1의 토큰으로 존재합니다 (`of`/`into`와 함께 사용):
...
`KMixedUnitInstance`로 남아 있으며 `toElectricFieldStrength()`로 좁혀집니다 (이는 정규 형식만 인식하고 그렇지 않으면 `IllegalStateException`을
발생시킵니다). 모든 경로는 값-동등합니다.
