...
그 위에 다른 모든 "순수" 단위 래퍼와 동일한 표면(`value`/`valueIn`/`+`/`-`/`*`/`/`/`toString`/
`toKUnitInstance`)을 제공하므로, 시간 값은 범용 혼합 단위 엔진에 그대로 연결됩니다(예: `length / time` =
속도). 값은 항상 초로 정규화되어 저장됩니다.

`Duration`은 항상 단순한 지속 시간만 나타내므로, 시간 값은 항상 지수 1입니다 - time² 또는 1/time 래퍼는
없습니다(곱하기/나누기는 길이와 마찬가지로 원시 `KUnitInstance`로 "빠져나갑니다"). 따라서
`KUnitInstance.toKTimeUnit()`은 **지수 1의** 단일 `KTimeUnit` 항만 허용합니다.
...
t.valueIn(KTimeUnit.HOUR)    // 2.0 (시간으로 다시 읽기)
...
val ratio = 10.seconds() / 2.seconds()           // KUnitInstance: value=5.0, 무차원
```
...
것과도 범용 infix 생성 함수와 `with`(valueIn/toString 대상용)를 사용해 결합할 수 있습니다. 하위 초(및
일 이상) 단위는 이렇게 표현합니다.
...
// 생성: "5 milli seconds" -> KPrefixBuilder -> KUnitInstance -> KTimeUnitInstance
...
!!! note "Duration 범위"
    ...
    반올림됩니다. 범용 `KUnitInstance`/접두어 계층 자체는 `Double` 기반이라 영향을 받지 않으며,
