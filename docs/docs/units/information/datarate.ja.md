...
パッケージ: `org.pcsoft.framework.kunit.it.datarate`
基本単位: **バイト毎秒**(`KDataRateUnit.BASE == KDataRateUnit.BYTES_PER_SECOND`)

種別: **構成された単位**

データ転送率は **構成された**単位です ([速度](../kinematics/speed.md) に続く2つ目): 単一の「実在する」量ではなく、
...
します。値は、どの単位やストレージ/時間の組み合わせから作成されたかに関係なく、常にバイト毎秒に正規化されて 保存されます。
...
データ転送率 *とは*、ストレージ量を時間で割ったものです。3つの量 — ストレージ、時間、データ転送率 — の間を 単純な `*` と `/`
で行き来でき、各結果は **強く型付け**されます。
...
`KMixedUnitInstance.toDataRate()` は、ちょうど1つの指数 `+1` のストレージ項と1つの指数 `-1` の時間項を 要求します。`B²`
(ストレージの2乗)、`B·s⁻²`、`B·s` の形はデータ転送率ではありません — 変換は
...
データ転送率グループは [ストレージ](storage.md) グループの接頭辞ポリシーを反映します (その分子はストレージ量 です): 分子は
**増大** SI ビルダー (`kilo`、`mega` など)または **2進**ビルダー (`kibi`、`mebi` など)を使用 します。縮小ビルダーには
`bytes`/`bits` プロパティがないため、`milli.bytes / seconds` はコンパイルされません。
