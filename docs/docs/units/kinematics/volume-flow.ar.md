...
`distance³ · time⁻¹` (`m³/s`). يغلّف `KVolumeFlowUnitInstance` كائن `KMixedUnitInstance` مكوّن من حدّين
بالضبط — واحد `KDistanceUnit.BASE` (متر) بالأس `+3`، وآخر `KTimeUnit.BASE` (ثانية) بالأس `-1`. تُطبَّع
القيمة دائمًا إلى متر مكعب لكل ثانية، بصرف النظر عن الوحدة أو تركيبة الحجم/الزمن التي أُنشئت منها.

خلافًا للطاقة أو القدرة، لا يملك التدفق الحجمي **أي** بُعد كتلة، لذا فإن القيمة المخزَّنة *هي* القراءة
بـ`m³/s` — دون وجود أي جسر غرام/كيلوغرام.
...
تُضخّ مضخّة حديقة 300 l/min في خزّان سعة 5 m³. كم من الوقت يستغرق ملء الخزّان، وما معدّل التدفّق
بالوحدات التي تستخدمها بطاقة بيانات المضخّة؟
...
تتعرّف `toVolumeFlow()` على **الصيغة القياسية فقط** (حدّ واحد من `KDistanceUnit` بالأس `+3` وحدّ واحد
من `KTimeUnit` بالأس `-1`)؛ وأي تعبير مكافئ يُختزَل إليها تلقائيًا. أما الشكل الخاطئ فيرمي
