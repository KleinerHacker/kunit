...
운동학은 **운동**에 대한 기술입니다 — 얼마나 멀리, 얼마나 오래, 얼마나 빠르게, 그리고 운동의 비율 자체가 어떻게 변하는지 — 아직 그 배후의 힘은 묻지 않습니다
(그것은 [역학](../mechanics/overview.md)
주제입니다). KUnit은 이 분야를 2개의 **네이티브** 기본량과, 그것들로부터 **구성된** 3개의 양으로 모델링하므로, 고전적인 운동 공식이 강한 타입을 유지한 채 일반적인 `*`와 `/` 식이 됩니다.
...
속도는 거리÷시간, 가속도는 속도÷시간, 주파수는 시간의 역수입니다. KUnit은 각 조합에 대해 올바른 **타입이 지정된** 양을 반환합니다 — 원시 혼합 단위를 직접 조립할 필요가 없습니다.
...
자동차가 **120 km**를 **1.5 h** 만에 주행합니다. 평균 속도는 `v = s / t`이며, 그 속도에 소요 시간을 곱하면 다시 주행 거리가 나옵니다:
