...
(`1 Hz = 1/s`): يغلّف `KFrequencyUnitInstance` حدّ `KFrequencyUnit.HERTZ` واحدًا، مُطبَّعًا دائمًا إلى الهرتز.

ولأنّ التردد مقلوب الزمن، فإنّ سلوكه عبر المجموعات مُعرَّف بأنّه **عكس الزمن تمامًا**: الضرب في تردد يتصرّف كالقسمة على
زمن، والقسمة على تردد كالضرب في زمن.
...
عجلة محيطها **2 m** تدور بسرعة **5 دورات في الثانية**. ضرب محيطها في تردد الدوران يعطي السرعة السطحية (سرعة التماس) —
`length * frequency = speed`، وهو عكس `length / time = speed` المألوف:
...
ارفع قيمة لقوّة عددية صحيحة بالمعامِل `pow` (لا يملك Kotlin معامِل `^` قابلًا للتحميل الزائد). لمجموعة التردد يُعيد
`pow` وحدة `KMixedUnitInstance` عامّة (لا نوع قوّة مُبعَّد للتردد):
...
يقبل التردد **أي** مقدار، فيمكن دمج كل باني بادئة SI (`quetta` … `quecto`) مع كل وحدة تردد عبر الوصول إلى الخاصّية.
`kilo.hertz` هو kHz، و`mega.hertz` هو MHz، و`giga.hertz` هو GHz.
