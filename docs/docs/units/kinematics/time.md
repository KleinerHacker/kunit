...
Because a `Duration` only ever represents a plain duration, a time value is always exponent 1 - there is no time² or
1/time wrapper (multiplying/dividing "escapes" to a raw `KMixedUnitInstance`, exactly like length). Consequently
`KMixedUnitInstance.toTime()` accepts only a single `KTimeUnit` term **at exponent 1**.
...
Only physical time scales are modeled; calendar-based units (week, year) are intentionally omitted, since they are
defined by calendars rather than by a fixed physical quantity. Each `Token` is a value-1
`KTimeUnitInstance` used with `of` (build) and `into` (read).

Sub-second scales (millisecond, microsecond, nanosecond, ...) are **not** dedicated units - they are reached generically
via the SI prefix builders on `seconds` (see [SI prefixes](#si-prefixes) below).
...
Any time unit can be combined with any of the 24 SI prefix **builders** (`kilo`, `milli`, `micro`, …; root package) via
property access, producing a value-1 template for `of`/`into`. This is how sub-second scales are expressed. Note that
the `Duration` backing limits the representable range (see the note below), so extreme prefixes on a multi-second base
