...
الحزمة: `org.pcsoft.framework.kunit.mechanic.density`
...
`KDensityUnitInstance` نسخةَ `KMixedUnitInstance` من حدّين — `KMassUnit.BASE` (غرام) عند `+1`، و`KDistanceUnit.BASE`
(متر) عند `-3`. القيمة المخزّنة هي قيمة المكوّن الخام القائمة على الغرام؛ والقراءات بـ kg/m³ تُقسَم على معامل ثابت.

## بناء كثافة

كالكثافة السطحية، **لا رمز مجرّد** للكثافة — فكل كتابة (kg/m³، g/cm³، …) نسبة. ابنِها كتعبير أو عبر المعامِل المحكوم
بالنوع `mass / volume`، واقرأها ثانيةً بـ `into` مقابل مثل هذا التعبير:
