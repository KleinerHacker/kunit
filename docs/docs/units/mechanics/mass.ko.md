...
백킹도 없는 **단순한 1차원** 그룹입니다. `KMassUnitInstance` 는 단일 `KMassUnit.GRAM` 항을 감싸며 항상 그램으로 정규화하여 저장합니다.

기준 단위는 의도적으로 **킬로그램이 아니라 그램** 입니다. 킬로그램은 전용 단위가 아니라 단순히 SI 접두사
`kilo` 를 그램에 적용한 `kilo.grams` 입니다. 그램의 모든 십진 자릿수 (밀리그램, 킬로그램 등)는 SI 접두사를 통해 동일한 일반적 방식으로 얻어집니다.
...
질량은 **모든** 자릿수를 허용하므로 모든 SI 접두사 빌더 (`quetta` … `quecto`)를 프로퍼티 접근으로 모든 질량 단위와 결합할 수 있습니다. 킬로그램은 정확히 `kilo.grams`, 밀리그램은
`milli.grams` 입니다.
