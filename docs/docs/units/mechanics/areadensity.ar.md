...
الحزمة: `org.pcsoft.framework.kunit.mechanic.areadensity`
...
`KMassUnit.BASE` (غرام) عند `+1`، و`KDistanceUnit.BASE` (متر) عند `-2`. القيمة المخزّنة هي قيمة المكوّن الخام القائمة
على الغرام؛ والقراءات بـ kg/m² تُقسَم على معامل ثابت.

## بناء كثافة سطحية

كالكثافة، **لا رمز مجرّد** للكثافة السطحية — فكل كتابة (kg/m²، g/mm²، …) نسبة. ابنِها كتعبير أو عبر المعامِل المحكوم
بالنوع `mass / area`، واقرأها ثانيةً بـ `into` مقابل مثل هذا التعبير:
