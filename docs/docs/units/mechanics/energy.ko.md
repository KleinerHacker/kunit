...
에너지는 기술적으로 여러 주제 영역에 걸쳐 나타나는 **하나의** 물리량입니다. 이 페이지는 그 *역학적* 해석 — **일 (work)**, `W = F · s`를 설명합니다. 동일한 Kotlin 그룹은 다른
영역에서도
[에너지 (전기)](../electrical/energy.md)와 [에너지 (열역학)](../thermodynamics/energy.md)에서 문서화되어 있습니다.

## 에너지 만들기

이름이 붙은 토큰으로 에너지를 만들거나, 아래의 분해식으로부터 만들 수 있습니다. 이름이 붙은 단위는 값 1의 토큰으로 존재합니다 (`of`/`into`와 함께 사용):
...
| 표현식               | 결과 타입             | 의미                                                                   |
|----------------------|-----------------------|------------------------------------------------------------------------|
| `force * length`     | `KEnergyUnitInstance` | 역학적 일 `W = F · s` (교환 가능)                                      |
| `power * time`       | `KEnergyUnitInstance` | 시간에 걸친 전력으로부터 계산된 일 `W = P · t` (교환 가능)             |
...
역연산자는 전력, 시간, 에너지를 함께 묶습니다:

| 표현식           | 결과 타입            | 의미                                      |
|------------------|----------------------|-------------------------------------------|
| `energy / time`  | `KPowerUnitInstance` | `P = W / t` ([전력(역학)](power.md) 참조) |
...
| 수학        | Kotlin                                            | 의미                                         |
|-------------|---------------------------------------------------|----------------------------------------------|
| `J`         | `joules`                                          | 에너지(일), 기본 단위(이름이 붙은 토큰, 줄)  |
| `F · s`     | `(100 of newtons) * (5 of meters)`                | 힘과 길이로부터 계산된 역학적 일             |
