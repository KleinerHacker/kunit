...
패키지: `org.pcsoft.framework.kunit.mechanic.density`
기본 단위: **킬로그램 매 세제곱미터**(`KDensityUnit.BASE == KDensityUnit.KILOGRAM_PER_CUBIC_METER`)

유형: **구성된 단위**

밀도 (질량 밀도)는 **구성된** 단위로, 조합 `mass · length⁻³`(`kg/m³`)입니다. `KDensityUnitInstance` 는 두 항 — 지수 `+1` 의 `KMassUnit.BASE`(그램)
와 지수 `-3` 의 `KDistanceUnit.BASE`(미터) — 을 감쌉니다. 저장 값은 원시 그램 기준 성분 값이며 kg/m³ 로 읽을 때 고정 인자로 나눕니다.
