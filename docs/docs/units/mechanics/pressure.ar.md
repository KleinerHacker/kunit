...
`KPressureUnitInstance` نسخةَ `KMixedUnitInstance` من ثلاثة حدود — `KMassUnit.BASE` (غرام) عند `+1`، و
`KDistanceUnit.BASE` (متر) عند `-1`، و`KTimeUnit.BASE` (ثانية) عند `-2`. وكما في القوة، القيمة المخزّنة هي قيمة المكوّن
الخام القائمة على الغرام، والقراءات بالباسكال تُقسَم على معامل ثابت.
