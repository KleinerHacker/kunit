...
Package: `org.pcsoft.framework.kunit.mechanic.density`
...
`KDistanceUnit.BASE` (meter) at `-3`. The stored value is the raw gram-based component value; readings in kg/m³ divide
by a fixed factor.

## Building a density

Density has **no bare token** — every spelling (kg/m³, g/cm³, …) is a ratio. Build it as an expression or via the typed
`mass / volume` operator, and read it back with `into` against such an expression:
