...
تُنمذج مجموعة الكتلة مقدارًا من الكتلة. وهي مجموعة **بسيطة أحادية البُعد** (بلا أنواع فرعية مخصّصة للأُسّ كمجموعة
المسافة، وبلا دعامة `Duration` كمجموعة الزمن): يغلّف `KMassUnitInstance` حدّ `KMassUnit.GRAM`
...
`kilo.grams`، أي بادئة SI `kilo` مطبَّقة على الغرام. ويُوصَل إلى كل مقدار عشري للغرام (ميلي غرام، كيلوغرام، …) بالطريقة
العامّة نفسها عبر بادئات SI.
...
تقارن `==`، `!=`، `<`، `<=`، `>`، `>=` القيمة المُطبَّعة (بالغرامات) لقيمتَي `KMassUnitInstance`. و`equals` بحسب المقدار
المُطبَّع، فـ `(1 of kilo.grams) == (1000 of grams)`.

## القوى بـ `pow`

ارفع قيمة لقوّة عددية صحيحة بالمعامِل `pow` (لا يملك Kotlin معامِل `^` قابلًا للتحميل الزائد). لمجموعة الكتلة يُعيد
`pow` وحدة `KMixedUnitInstance` عامّة (لا نوع قوّة مُبعَّد للكتلة):
...
تقبل الكتلة **أي** مقدار، فيمكن دمج كل باني بادئة SI (`quetta` … `quecto`) مع كل وحدة كتلة عبر الوصول إلى الخاصّية.
الكيلوغرام هو بالضبط `kilo.grams`؛ والميلي غرام هو `milli.grams`.
