...
密度 (質量密度)は **構成された**単位で、合成 `mass · length⁻³`(`kg/m³`)です。`KDensityUnitInstance` は2つの 項 — 指数 `+1`
の `KMassUnit.BASE`(グラム)と指数 `-3` の `KDistanceUnit.BASE`(メートル) — をラップします。 保存される値は生のグラム基準の成分値で、kg/m³
での読み取りは固定係数で除算されます。
