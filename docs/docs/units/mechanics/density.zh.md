...
包: `org.pcsoft.framework.kunit.mechanic.density`
基本单位: **千克每立方米**(`KDensityUnit.BASE == KDensityUnit.KILOGRAM_PER_CUBIC_METER`)

类型： **构造单位**

密度 (质量密度)是一个 **构造**单位,即组合 `mass · length⁻³`(`kg/m³`)。`KDensityUnitInstance` 包装两项—— 指数为 `+1` 的
`KMassUnit.BASE`(克)和指数为 `-3` 的 `KDistanceUnit.BASE`(米)。存储值为原始的以克为基准的 分量值,以 kg/m³ 读取时除以固定因子。
