...
* Sub-package `org.pcsoft.framework.kunit.time` (physical quantity time):
  ...
  * `KTimeUnitInstance` - a 100 % wrapper around `java.time.Duration` (the `Duration` is the source of
    ...
    surface (`value`, `valueIn`, `+`/`-`/`*`/`/`, comparison operators, `toString`, `toKUnitInstance`,
    `toDuration`) plus `KUnitInstance.toKTimeUnit()`/`Duration.toKTimeUnit()`. Time is always exponent 1
